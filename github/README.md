...
1. Install the GitHub app https://github.com/apps/awmate-agent. Make sure it is installed on the target repository.
2. Add the following workflow file to `.github/workflows/awmate.yml` in your repo. Set the appropriate `model` and required API keys in `env`.
...
This is an early release. If you encounter issues or have feedback, please create an issue at https://github.com/anomalyco/awmate/issues.
...
2. Run:
   ...
   MODEL=anthropic/claude-sonnet-4-20250514 \
     ...
     bun /path/to/awmate/github/index.ts
   ```
   ...
   - `/path/to/awmate`: Path to your cloned opencode repo. `bun /path/to/awmate/github/index.ts` runs your local version of `opencode`.
