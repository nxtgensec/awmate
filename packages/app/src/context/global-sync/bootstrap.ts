import type {
  ...
  ReferenceInfo,
  Session,
} from "@awmate/sdk/v2/client"
import { showToast } from "@/utils/toast"
import { getFilename } from "@awmate/core/util/path"
import { retry } from "@awmate/core/util/retry"
...
import { NormalizedProviderListResponse } from "@awmate/session-ui/context"
...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: AwmateClient) =>
  queryOptions({
    queryKey: [scope, "config"],
    queryFn: () => retry(() => sdk.global.config.get().then((x) => x.data!)),
  })

export const loadProjectsQuery = (scope: ServerScope, sdk: AwmateClient) =>
  queryOptions({
...
export const loadProvidersQuery = (scope: ServerScope, directory: string | null, sdk: AwmateClient) =>
  queryOptions({
    queryKey: [scope, directory, "providers"],
    queryFn: () => retry(() => sdk.provider.list().then((x) => normalizeProviderList(x.data!))),
  })

export const loadAgentsQuery = (scope: ServerScope, directory: string | null, sdk: AwmateClient) =>
  queryOptions({
    queryKey: [scope, directory, "agents"],
    queryFn: () => retry(() => sdk.app.agents().then((x) => normalizeAgentList(x.data))),
  })

export const loadPathQuery = (scope: ServerScope, directory: string | null, sdk: AwmateClient) =>
  queryOptions<Path>({
    queryKey: [scope, directory, "path"],
    queryFn: () => retry(() => sdk.path.get().then((x) => x.data!)),
  })

export const loadReferencesQuery = (scope: ServerScope, directory: string, sdk: AwmateClient) =>
