  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://awmate.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://awmate.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://awmate.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://awmate.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://awmate.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://awmate.example.com"),
      ),
    ).toEqual({
