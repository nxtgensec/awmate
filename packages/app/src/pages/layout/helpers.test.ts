...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("awmate://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("awmate://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("awmate://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("awmate://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("awmate://open-project")).toBeUndefined()
    expect(parseDeepLink("awmate://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "awmate://open-project?directory=/a",
      "awmate://other?directory=/b",
      "awmate://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("awmate://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("awmate://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("awmate://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("awmate://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "awmate://new-session?directory=/a",
      "awmate://open-project?directory=/b",
      "awmate://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __AWMATE__: {
        deepLinks: ["awmate://open-project?directory=/a"],
      },
    } as unknown as Window & { __AWMATE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["awmate://open-project?directory=/a"])
