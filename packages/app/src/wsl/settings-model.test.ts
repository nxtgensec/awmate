...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when AWMate needs attention", () => {
    expect(wslAwmateAction(undefined)).toBeUndefined()
    expect(
      wslAwmateAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    expect(
      wslAwmateAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/awmate",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when AWMate is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        awmateChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.awmate/bin/awmate",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installAwmate" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-awmate")
  })
