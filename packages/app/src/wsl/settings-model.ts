...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-awmate" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslAwmateAction(check?: WslAwmateCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install AWMate"
  if (check.matchesDesktop === false) return "Update AWMate"
}
...
export function addServerViewModel(input: {
  ...
  const opencodeCheck = selectedDistro ? (state?.awmateChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.awmateChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.error) return { label: { key: "wsl.onboarding.installAwmate" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  opencodeCheck: WslAwmateCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingAwmate =
    input.state?.job?.kind === "install-awmate" && input.state.job.distro === input.selectedDistro
  if (!ready || probingAwmate) {
    return {
      variant: "contrast",
...
function addServerOpencodeReady(check: WslAwmateCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.awmateChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.awmateChecks[item.name]) return [`opencode:${item.name}`]
    return []
  })
