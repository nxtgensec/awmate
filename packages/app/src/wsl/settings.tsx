...
import { Tag } from "@awmate/ui/v2/badge-v2"
...
import { wslAwmateAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.awmateChecks[item.config.distro]
          const opencodeAction = () => wslAwmateAction(check())
          const busy = () => wsl.data?.job?.kind === "install-awmate" && wsl.data.job.distro === item.config.distro
          return (
                ...
                <Show when={opencodeAction()}>
                  {(label) => (
                    <ButtonV2
                      ...
                      onClick={() => api && request.mutate(() => api.installAwmate(item.config.distro))}
                    >
