...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      AWMATE_CLI_NAME: `'${binary}'`,
      AWMATE_MODELS_DEV: modelsData,
      AWMATE_CHANNEL: `'${Script.channel}'`,
      AWMATE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/awmate.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
