...
This is the living design and status document for `@awmate/codemode` and its existing V2 AWMate adapter.
...
`packages/awmate/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
