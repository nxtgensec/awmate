...
import type { IntegrationOAuthMethodRegistration } from "@awmate/plugin/v2/effect/integration"
import { define } from "@awmate/plugin/v2/effect/plugin"
import type { CredentialValue } from "@awmate/sdk/v2/types"
...
const defaultServer = "https://console.awmate.ai"
const clientID = "awmate-cli"
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load AWMate provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
