...
import { ToolFailure } from "@awmate/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@awmate/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.AWMATE_WEBSEARCH_PROVIDER === "exa" || process.env.AWMATE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.AWMATE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("AWMATE_EXPERIMENTAL") || truthy("AWMATE_ENABLE_EXA") || truthy("AWMATE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("AWMATE_ENABLE_PARALLEL") || truthy("AWMATE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `awmate/${InstallationVersion}`,
