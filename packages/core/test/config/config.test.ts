...
import { Policy } from "@awmate/core/policy"
...
describe("Config", () => {
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "awmate.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "awmate.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "awmate.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "awmate.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .awmate configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".awmate"), { recursive: true })
            await fs.mkdir(path.join(directory, ".awmate"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "awmate.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "awmate.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "awmate.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "awmate.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "awmate.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".awmate", "awmate.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".awmate", "awmate.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".awmate")),
              AbsolutePath.make(path.join(directory, ".awmate")),
            ])
