...
// test/preload.ts pins AWMATE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.AWMATE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.AWMATE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.AWMATE_MODELS_PATH = undefined
  Flag.AWMATE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.AWMATE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.AWMATE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.AWMATE_DISABLE_MODELS_FETCH = false
        }),
