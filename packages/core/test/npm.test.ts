...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@awmate/acme")).toBe("@awmate/acme")
    expect(Npm.sanitize("@awmate/acme@1.0.0")).toBe("@awmate/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/awmate/acme.git"
    const expected = win ? "acme@git+https_//github.com/awmate/acme.git" : spec
