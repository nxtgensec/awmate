...
import { ModelsDevPlugin } from "@awmate/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.AWMATE_MODELS_PATH,
          disabled: Flag.AWMATE_DISABLE_MODELS_FETCH,
        }
        Flag.AWMATE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.AWMATE_MODELS_PATH = previous.path
          Flag.AWMATE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
