...
import { PluginHost } from "@awmate/core/plugin/host"
import { OpencodePlugin } from "@awmate/core/plugin/provider/opencode"
...
describe("OpencodePlugin", () => {
  ...
  it.live("loads providers and models from the connected AWMate server", () =>
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ AWMATE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
  ...
  it.effect("uses AWMATE_API_KEY as credentials", () =>
    withEnv({ AWMATE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ AWMATE_API_KEY: undefined, CUSTOM_AWMATE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("opencode"),
            method: { type: "env", names: ["CUSTOM_AWMATE_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
