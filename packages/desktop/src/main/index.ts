...
const main = Effect.gen(function* () {
  ...
  process.env.AWMATE_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.awmate.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `awmate-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "AWMate Dev")
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.AWMATE_PORT
    if (fromEnv) {
