...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.awmate.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `awmate.settings.dat` is special: it maps to the `awmate.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"awmate.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // awmate.settings.dat → the electron settings store ("awmate.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "awmate.global.dat").
  const storeName = filename === "awmate.settings.dat" ? "awmate.settings" : filename
