...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectAwmateVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectAwmateVersion("1.14.35", "1.16.2")).toThrow(
    "AWMate update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating AWMate", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.awmate/bin/awmate",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, awmateChecks: {} })
})
...
test("ignores stale background AWMate checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().awmateChecks).toEqual({})
})

test("ignores stale startup AWMate checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking AWMate", async () => {
  persistedServers = []
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveAwmate: async (distro) => {
      opencode.push(distro)
      return "/home/me/.awmate/bin/awmate"
    },
  })
  ...
  expect(Object.keys(controller.getState().awmateChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check AWMate in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const opencode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().awmateChecks)).toEqual(["Debian"])
})
