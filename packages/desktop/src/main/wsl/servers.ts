...
import { expectAwmateVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveAwmate?: typeof resolveWslAwmate
...
export function createWslServersController(
  ...
  const setAwmateCheck = (distro: string, check: WslAwmateCheck) => {
    setState({
      awmateChecks: {
        ...state.awmateChecks,
        [distro]: check,
  ...
  const checkAwmate = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveAwmate ?? resolveWslAwmate)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return awmateCheck(distro, resolved, version, appVersion)
  }

  const refreshAwmateCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setAwmateCheck(distro, await checkAwmate(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const awmateChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.awmateChecks[distro])
        .map(async (distro) => [distro, await checkAwmate(distro, opts)] as const),
    )
    if (awmateChecks.length) {
      setState({ awmateChecks: { ...state.awmateChecks, ...Object.fromEntries(awmateChecks) } })
    }
  }
  ...
  const refreshAwmateCheckBackground = (id: string, distro: string) => {
    void checkAwmate(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setAwmateCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl awmate check failed", { id, distro, message })
      })
  }

  const refreshAwmateChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkAwmate(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setAwmateCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl awmate check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshAwmateCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installAwmate(name: string) {
      await runJob({ kind: "install-awmate", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslAwmate(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "AWMate installation failed")
        }
        await refreshAwmateCheck(name, { signal: abort.signal })
        expectAwmateVersion(state.awmateChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.awmateChecks, distro) : {}),
      })
    },
...
function awmateCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "awmate is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "awmate is installed but could not run",
    }
  }
  return {
    distro,
