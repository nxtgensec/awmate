...
import { type WslCommandLine, resolveWslAwmate, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const awmate = await resolveWslAwmate(distro)
  if (!awmate) throw new Error(`AWMate is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "awmate"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export AWMATE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export AWMATE_CLIENT=desktop",
    `export AWMATE_SERVER_USERNAME=${shellEscape(username)}`,
    `export AWMATE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(awmate)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
