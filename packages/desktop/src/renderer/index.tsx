    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.AWMATE_CHANNEL === "prod" &&
...
const deepLinkEvent = "awmate:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__AWMATE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `awmate.desktop.window.${windowID}.last-active-url`
}
...
function LoadingSplash() {
  return (
    <div class="h-dvh w-screen flex flex-col items-center justify-center bg-background-base">
      <div class="text-20-bold text-text-weaker animate-pulse">AWMate</div>
    </div>
  )
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("awmate.global.dat").getItem("language")
