...
> current private `@awmate/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@awmate/ai`
...
- Secondary audience: AWMate and other durable agent runtimes
...
import { LLM } from "@awmate/ai"
import { OpenAI } from "@awmate/ai/providers/openai"
...
import { LLM, Tool } from "@awmate/ai"
...
AWMate and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@awmate/ai/promise"
import { OpenAI } from "@awmate/ai/providers/openai"
...
```ts
import { LLMSchema } from "@awmate/ai/schema"
...
```ts
import { Provider, Protocol } from "@awmate/ai/provider"
...
@awmate/ai/providers/openai
@awmate/ai/providers/anthropic
@awmate/ai/providers/google
...
```
...
AWMate should migrate to `generateTurn` / `streamTurn`, preserving its durable
