...
- The server still lives in `packages/awmate`.
...
Until enough shared service code lives outside `packages/awmate`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/awmate`

It should not import `packages/awmate` services while `packages/awmate`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/awmate` implementation details.
