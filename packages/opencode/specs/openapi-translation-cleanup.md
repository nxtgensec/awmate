...
Trim `packages/awmate/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/awmate/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/awmate`.
- `[x]` `bun typecheck` from `packages/awmate`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/awmate/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/awmate`.
- `[x]` `bun dev generate > /tmp/awmate-openapi.json` from `packages/awmate`.
...
- Edit `packages/awmate/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/awmate-openapi.json` from `packages/awmate`.
...
- Check source schemas in files like `packages/awmate/src/session/schema.ts`, `packages/awmate/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/awmate/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/awmate/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
