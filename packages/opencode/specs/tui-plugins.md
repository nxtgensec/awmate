...
- Author package entrypoint is `@awmate/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/awmate-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@awmate/plugin/tui`.
- `@awmate/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@awmate/plugin/tui"
...
- The value is a semver range checked against the running AWMate version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.awmate` is absent, no check is performed (backward compatible).
...
- `awmate plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `awmate plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.awmate` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.awmate`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.awmate`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.awmate` dir or `OPENCODE_CONFIG_DIR`, AWMate installs `@awmate/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@awmate/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus AWMate's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
AWMate registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@awmate/plugin/tui`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `awmate.default`.
...
- Local plugins persist installed themes under the local `.awmate/themes` area near the plugin config source.
...
- Local smoke plugin: `.awmate/plugins/tui-smoke.tsx`
- Local vim plugin: `.awmate/plugins/tui-vim.tsx`
- Local smoke config: `.awmate/tui.json`
- Local smoke theme: `.awmate/plugins/smoke-theme.json`
