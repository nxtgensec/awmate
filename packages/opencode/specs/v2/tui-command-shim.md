...
Delete `packages/awmate/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/awmate/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/awmate/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/awmate`
- TUI plugin loader tests in `packages/awmate` if runtime plugin API wiring changed
