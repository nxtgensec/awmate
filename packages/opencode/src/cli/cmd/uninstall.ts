...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall awmate and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall AWMate")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g awmate-ai",
      pnpm: "pnpm uninstall -g awmate-ai",
      bun: "bun remove -g awmate-ai",
      yarn: "yarn global remove awmate-ai",
      brew: "brew uninstall awmate",
      choco: "choco uninstall awmate",
      scoop: "scoop uninstall awmate",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using AWMate!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# awmate") || content.includes(".awmate/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".awmate/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".awmate/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".awmate"))
    ) {
      continue
    }
