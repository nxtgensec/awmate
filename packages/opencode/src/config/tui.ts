...
import { isRecord } from "@awmate/tui/util/record"
...
import { TuiKeybind } from "@awmate/tui/config/keybind"
...
import { TuiConfig } from "@awmate/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@awmate/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old awmate.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.awmate`
  // folders between cwd and home, and AWMATE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.AWMATE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit AWMATE_TUI_CONFIG override, if set.
  if (Flag.AWMATE_TUI_CONFIG) {
    const configFile = Flag.AWMATE_TUI_CONFIG
  ...
  // 4. `.awmate` directories (and AWMATE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".awmate") || dir === Flag.AWMATE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".awmate") && dir !== Flag.AWMATE_CONFIG_DIR) continue
