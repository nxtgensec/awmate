...
import { IdeEvent } from "@awmate/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["AWMATE_CALLER"] === "vscode" || process.env["AWMATE_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.awmate"], {
    nothrow: true,
  })
