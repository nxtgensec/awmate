...
export function userAgent(client = "cli") {
  return `awmate/${InstallationChannel}/${InstallationVersion}/${client}`
}
...
const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getBrewFormula = Effect.fnUntraced(function* () {
      const tapFormula = yield* text(["brew", "list", "--formula", "anomalyco/tap/awmate"])
      if (tapFormula.includes("opencode")) return "anomalyco/tap/awmate"
    ...
    const upgradeCurl = Effect.fnUntraced(
      function* (target: string) {
        const response = yield* httpOk.execute(HttpClientRequest.get("https://awmate.ai/install"))
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        ...
        for (const check of checks) {
          ...
          const installedName =
            check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "opencode" : "awmate-ai"
      ...
      latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "brew") {
          ...
          const response = yield* httpOk.execute(
            HttpClientRequest.get("https://formulae.brew.sh/api/formula/awmate.json").pipe(
        ...
        if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              `${yield* NpmConfig.registry(process.cwd())}/awmate-ai/${InstallationChannel}`,
        ...
        if (detectedMethod === "scoop") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/awmate.json",
        ...
        const response = yield* httpOk.execute(
          HttpClientRequest.get("https://api.github.com/repos/anomalyco/awmate/releases/latest").pipe(
            HttpClientRequest.acceptJson,
          ),
        )
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          case "curl":
            ...
            break
          case "npm":
            upgradeResult = yield* run(["npm", "install", "-g", `awmate-ai@${target}`])
            break
          case "pnpm":
            upgradeResult = yield* run(["pnpm", "install", "-g", `awmate-ai@${target}`])
            break
          case "bun":
            upgradeResult = yield* run(["bun", "install", "-g", `awmate-ai@${target}`])
            break
          case "brew": {
            const formula = yield* getBrewFormula()
