...
export const DEPRECATED_PLUGIN_PACKAGES = ["awmate-openai-codex-auth", "opencode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, awmateVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(awmateVersion) || semver.major(awmateVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.awmate
  if (typeof range !== "string") return
  if (!semver.satisfies(awmateVersion, range)) {
    throw new Error(`Plugin requires awmate ${range} but running ${awmateVersion}`)
  }
}
