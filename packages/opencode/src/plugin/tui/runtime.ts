...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@awmate/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@awmate/core/effect/app-node-builder"
import { errorData, errorMessage } from "@awmate/tui/util/error"
...
import { hasTheme, upsertTheme } from "@awmate/tui/context/theme"
import { Global } from "@awmate/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@awmate/core/util/flock"
import { Flag } from "@awmate/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@awmate/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@awmate/core/v1/config/plugin"
import { createCommandShim } from "@awmate/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@awmate/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".awmate"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".awmate", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".awmate", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.AWMATE_PURE ? [] : pluginOrigins
    if (Flag.AWMATE_PURE && pluginOrigins.length) {
    }
