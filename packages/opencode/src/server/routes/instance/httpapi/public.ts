...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for awmate serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(AWMateHttpApi)
...
export const PublicApi = AWMateHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "awmate",
    version: "1.0.0",
    description: "awmate api",
