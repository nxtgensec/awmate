...
const USER_AGENT = `awmate/${InstallationVersion}`
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("opencode")
        ? {
            ...(opencodeProjectID ? { "x-awmate-project": opencodeProjectID } : {}),
            "x-awmate-session": input.sessionID,
            "x-awmate-request": input.user.id,
            "x-awmate-client": input.flags.client,
