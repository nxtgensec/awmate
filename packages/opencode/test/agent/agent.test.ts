...
it.instance("plan agent denies edits except .awmate/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".awmate/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".awmate", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.AWMATE_TEST_HOME
      process.env.AWMATE_TEST_HOME = test.directory
