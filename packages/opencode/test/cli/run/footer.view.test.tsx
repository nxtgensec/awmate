...
import type { QuestionRequest } from "@awmate/sdk/v2"
import { AwmateKeymapProvider, registerAwmateKeymap } from "@awmate/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerAwmateKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerAwmateKeymap(keymap, renderer, tuiConfig)
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerAwmateKeymap(keymap, renderer, tuiConfig)
