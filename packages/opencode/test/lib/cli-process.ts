...
//   - AWMATE_CONFIG_CONTENT      : provider config inline, no files to find
//   - AWMATE_TEST_HOME           : pins os.homedir() → tmpdir
//   - AWMATE_DISABLE_PROJECT_CONFIG : skip walking up for awmate.json
//   - AWMATE_PURE                : skip external plugin discovery + install
//   - AWMATE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    AWMATE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    AWMATE_CONFIG_CONTENT: configJson,
    AWMATE_DISABLE_PROJECT_CONFIG: "1",
    AWMATE_PURE: "1",
    AWMATE_DISABLE_AUTOUPDATE: "1",
    AWMATE_DISABLE_AUTOCOMPACT: "1",
    AWMATE_DISABLE_MODELS_FETCH: "1",
    AWMATE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `awmate run`. New flags should
...
// `awmate serve` is a long-lived process — it never exits on its own.
...
// `awmate acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `awmate serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `awmate acp` and return a duplex JSON-RPC handle. Long-lived:
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          AWMATE_CONFIG_CONTENT: JSON.stringify({
    ...
    const serve = Effect.fn("opencode.serve")(function* (opts?: ServeOpts) {
      ...
      //   "awmate server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `awmate serve did not become ready within ${readyTimeoutMs}ms\n` +
