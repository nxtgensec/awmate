...
const original = {
  AWMATE_SERVER_PASSWORD: Flag.AWMATE_SERVER_PASSWORD,
  AWMATE_SERVER_USERNAME: Flag.AWMATE_SERVER_USERNAME,
  envPassword: process.env.AWMATE_SERVER_PASSWORD,
  envUsername: process.env.AWMATE_SERVER_USERNAME,
}
const auth = { username: "awmate", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.AWMATE_SERVER_PASSWORD = original.AWMATE_SERVER_PASSWORD
  Flag.AWMATE_SERVER_USERNAME = original.AWMATE_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.AWMATE_SERVER_PASSWORD
  else process.env.AWMATE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.AWMATE_SERVER_USERNAME
  else process.env.AWMATE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.AWMATE_SERVER_PASSWORD = auth.password
  Flag.AWMATE_SERVER_USERNAME = auth.username
  process.env.AWMATE_SERVER_PASSWORD = auth.password
  process.env.AWMATE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.AWMATE_SERVER_PASSWORD = undefined
  Flag.AWMATE_SERVER_USERNAME = auth.username
  delete process.env.AWMATE_SERVER_PASSWORD
  process.env.AWMATE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.AWMATE_DISABLE_DEFAULT_PLUGINS
    process.env.AWMATE_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.AWMATE_DISABLE_DEFAULT_PLUGINS
      else process.env.AWMATE_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
