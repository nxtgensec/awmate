...
import { createAwmateClient } from "@awmate/sdk/v2"
...
type Sdk = ReturnType<typeof createAwmateClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.AWMATE_SERVER_PASSWORD = input?.password
      Flag.AWMATE_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".awmate", "skills", "project-rest-skill", "SKILL.md"),
      `---
