...
describe("HttpApi workspace routing middleware", () => {
  it.live("proxies remote workspace HTTP requests through the selected workspace target", () =>
    Effect.gen(function* () {
      ...
      // This starts a second HTTP server that stands in for the awmate server
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const encodedHeaderDir = path.join(dir, "encoded header target")
      yield* serveProbe
      ...
      const encodedHeaderResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-opencode-directory", encodeURIComponent(encodedHeaderDir)),
        HttpClient.execute,
      )
      ...
      expect(yield* encodedHeaderResponse.json).toEqual({ directory: encodedHeaderDir, workspaceID: null })
    }),
  )
