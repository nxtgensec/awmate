...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.AWMATE_TEST ?? "")'), {
      env: {
        AWMATE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "AWMATE_TEST_SHELL"], {
      shell: true,
      env: {
        AWMATE_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("AWMATE_TEST_SHELL=ok")
  })
