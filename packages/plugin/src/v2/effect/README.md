# AWMate V2 Effect Plugin API

The Effect plugin API grants plugins two in-process capabilities:

- `hook` installs behavior at an AWMate extension point.
...
```ts
import { define } from "@awmate/plugin/v2/effect"
...
AWMate rebuilds the domain when a transform is registered or disposed. A rebuild starts from fresh domain state and runs every active transform in registration order.
