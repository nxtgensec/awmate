...
import { PromptInput } from "@awmate/schema/prompt-input"
import { Session } from "@awmate/schema/session"
import { Project } from "@awmate/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@awmate/schema/schema"
...
import { SessionEvent } from "@awmate/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this AWMate process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this AWMate process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
