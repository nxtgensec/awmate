...
test("embedded client uses the real router and handlers", async () => {
  ...
  const database = Flag.AWMATE_DB
  Flag.AWMATE_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Agent, Location, Model, AWMate, Prompt, Provider, Session, Tool } = await import("../src")
...
test("Location-owned runner events reach the ready global client", async () => {
  ...
  const database = Flag.AWMATE_DB
  Flag.AWMATE_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Location, AWMate, Prompt, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)
...
test("independent embedded hosts do not share live notifications", async () => {
  ...
  const database = Flag.AWMATE_DB
  Flag.AWMATE_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Agent, Location, AWMate, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)

  try {
    const program = Effect.gen(function* () {
      const first = yield* AWMate.create()
      const second = yield* AWMate.create()
...
test("embedded client is available as a Layer service", async () => {
  ...
  const database = Flag.AWMATE_DB
  Flag.AWMATE_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Location, AWMate, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)
  ...
  try {
    const created = await Effect.runPromise(
      ...
      }).pipe(Effect.provide(AWMate.layer), Effect.scoped),
    )
