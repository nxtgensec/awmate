...
import { AwmateClient } from "./gen/sdk.gen.js"
import { wrapClientError } from "../error-interceptor.js"
export { type Config as AwmateClientConfig, AwmateClient }
...
export function createAwmateClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html")
      throw new Error("Request is not supported by this version of AWMate Server (Server responded with text/html)")

    return response
  })
  client.interceptors.error.use(wrapClientError)
  return new AwmateClient({ client })
}
