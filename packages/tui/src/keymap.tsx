...
export const AWMATE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const AWMATE_MODE_KEY = "awmate.mode"

export const AwmateKeymapProvider = KeymapProvider
export const useAwmateKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type AwmateModeStack = ReturnType<typeof createAwmateModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, AwmateModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createAwmateModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(AWMATE_MODE_KEY, AWMATE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(AWMATE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(AWMATE_MODE_KEY, stack.at(-1)?.mode ?? AWMATE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? AWMATE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(AWMATE_MODE_KEY, undefined)
...
export function useAwmateModeStack() {
  return getAwmateModeStack(useAwmateKeymap())
}

export function getAwmateModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("AWMate mode stack is not registered for this keymap")
  return value
}
...
export function registerAwmateKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createAwmateModeStack(keymap)
