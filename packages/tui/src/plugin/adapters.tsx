import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@awmate/plugin/tui"
...
import type { useAwmateKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useAwmateKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getAwmateModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getAwmateModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
