...
import { getAwmateModeStack, AWMATE_BASE_MODE, AwmateKeymapProvider, registerAwmateKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerAwmateKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when opencode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getAwmateModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getAwmateModeStack(keymap).push("autocomplete")
