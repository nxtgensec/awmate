...
أنشئ الملف `.awmate/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات AWMate أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ AWMate:
...
- على مستوى النظام: `~/.config/awmate/commands/`
- لكل مشروع: `.awmate/commands/`
...
يتضمن AWMate عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
