...
يمكنك إضافة أدوات خارجية إلى AWMate باستخدام _Model Context Protocol_، أو MCP. يدعم AWMate خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات AWMate](https://awmate.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/awmate` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى AWMate تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم AWMate بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك AWMate المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `awmate mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن AWMate الرموز بشكل آمن في `~/.local/share/awmate/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل AWMate إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات AWMate مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط AWMate بحساب Sentry الخاص بك.
