---
title: المزوّدون
description: استخدام أي موفّر LLM في AWMate.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم AWMate ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات AWMate.
...
في `~/.local/share/awmate/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات AWMate.
...
AWMate Zen هي قائمة نماذج يوفّرها فريق AWMate وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع AWMate. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع AWMate Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر AWMate، ثم انتقل إلى [AWMate.ai/auth](https://awmate.ai/auth).
...
يعمل مثل أي مزوّد آخر في AWMate واستخدامه اختياري بالكامل.

---

## AWMate Go

AWMate Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق AWMate، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع AWMate.

1. شغّل الأمر `/connect` في TUI، واختر `AWMate Go`، ثم انتقل إلى [AWMate.ai/auth](https://awmate.ai/zen).
...
لاستخدام Amazon Bedrock مع AWMate:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل AWMate:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `awmate.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل AWMate غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين AWMate لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل AWMate بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات AWMate.
...
تخزين الرمز في مساحة تخزين مصادقة AWMate.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم AWMate نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر AWMate
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`awmate.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [AWMate-gitlab-auth](https://www.npmjs.com/package/awmate-gitlab-auth).

##### التهيئة

خصّص عبر `awmate.json`:
...
لاستخدام اشتراك GitHub Copilot مع AWMate:
...
لاستخدام Google Vertex AI مع AWMate:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر AWMate، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [AWMate-helicone-session](https://github.com/H2Shami/awmate-helicone-session) لتسجيل كل محادثة AWMate تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة AWMate مدرجة كجلسة مستقلة.
...
يمكنك ضبط AWMate لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط AWMate لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط AWMate لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع AWMate. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/awmate) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع AWMate:
...
3. انسخ مفتاح API لاستخدامه في AWMate.
...
6. **مهم**: قبل استخدام نماذج السحابة في AWMate، يجب سحب معلومات النموذج محليا:
...
AWMate Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق AWMate. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>AWMate Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **AWMate Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ AWMate.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات AWMate.
...
5. يمكنك أيضا تخصيصها عبر إعدادات AWMate. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع AWMate:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات AWMate. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع AWMate. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `awmate.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ AWMate بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `awmate auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات AWMate وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات AWMate.
