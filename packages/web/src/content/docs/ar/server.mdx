---
title: الخادم
description: تفاعل مع خادم AWMate عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `AWMate serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل AWMate استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `AWMate.local` |
...
عيّن `AWMate_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `AWMate`، أو عيّن `AWMate_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `AWMate serve` و `AWMate web`.
...
عند تشغيل `AWMate` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم AWMate للتفاعل مع AWMate برمجيا.
:::

تتيح هذه البنية لـ AWMate دعم عدة عملاء وتمكّنك من التفاعل مع AWMate برمجيا.

يمكنك تشغيل `AWMate serve` لبدء خادم مستقل. إذا كانت واجهة AWMate في terminal (TUI)
قيد التشغيل، فسيبدأ `AWMate serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات AWMate لـ [IDE](/docs/ide).
...
يعرض خادم AWMate واجهات API التالية.
