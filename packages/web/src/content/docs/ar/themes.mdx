...
مع AWMate يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم AWMate سمتنا `AWMate`.
...
- يريدون أن يطابق AWMate مظهر terminal لديهم
...
يدعم AWMate نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/awmate/themes/*.json` أو `$XDG_CONFIG_HOME/awmate/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.awmate/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.awmate/themes/*.json`
