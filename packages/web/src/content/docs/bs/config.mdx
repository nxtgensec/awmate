---
title: Konfiguracija
description: Korištenje AWMate JSON konfiguracije.
---

Možete konfigurirati AWMate koristeći JSON konfiguracijski fajl.

---

## Format

AWMate podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/awmate`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/awmate/awmate.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`awmate_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`awmate.json` u projektu) - postavke specifične za projekat
5. **`.awmate` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`awmate_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.awmate` i `~/.config/awmate` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/awmate` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu AWMate konfiguraciju u `~/.config/awmate/awmate.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/awmate/tui.json`.
...
Dodajte `awmate.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se AWMate pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `awmate_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `awmate_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.awmate` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.awmate`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`AWMate.ai/config.json`**](https://awmate.ai/config.json).

TUI konfiguracija koristi [**`AWMate.ai/tui.json`**](https://awmate.ai/tui.json).
...
Koristite `AWMate_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `awmate.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `AWMate serve` i `AWMate web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš AWMate server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `AWMate.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj AWMate konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, AWMate pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/awmate/agents/` ili `.awmate/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, AWMate će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`AWMate run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/awmate/commands/` ili `.awmate/commands/`. [Saznajte više ovdje](/docs/commands).
...
AWMate će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, AWMate **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju AWMate sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.awmate/plugins/` ili `~/.config/awmate/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite AWMate da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
