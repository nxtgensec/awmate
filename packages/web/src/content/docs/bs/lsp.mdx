---
title: LSP serveri
description: AWMate se integriše sa vašim LSP serverima.
---

AWMate se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

AWMate dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `AWMate_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i AWMate otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj AWMate konfiguraciji.
