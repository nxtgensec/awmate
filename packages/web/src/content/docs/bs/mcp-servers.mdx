...
Možete dodati vanjske alate u AWMate koristeći _Model Context Protocol_, ili MCP. AWMate podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [AWMate Config](https://awmate.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/awmate`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
AWMate automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, AWMate će:
...
Ako server zahtijeva autentifikaciju, AWMate će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `awmate mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, AWMate sigurno čuva tokene u `~/.local/share/awmate/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u AWMate, zajedno s ugrađenim alatima. Možete njima upravljati kroz AWMate konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete AWMate sa vašim Sentry nalogom.
