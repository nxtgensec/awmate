---
title: Dodaci
description: Napišite vlastite dodatke za proširenje AWMate.
---

Dodaci vam omogućavaju da proširite AWMate spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja AWMate.
...
- `.awmate/plugins/` - Dodaci na nivou projekta
- `~/.config/awmate/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/awmate/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/awmate/awmate.json`)
2. Konfiguracija projekta (`awmate.json`)
3. Globalni direktorij dodataka (`~/.config/awmate/plugins/`)
4. Direktorij dodataka projekta (`.awmate/plugins/`)
...
AWMate pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: AWMate SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje AWMate.
...
Spriječite AWMate da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u AWMate:
...
Pomoćnik `tool` kreira prilagođeni alat koji AWMate može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za AWMate zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://awmate.ai/docs/sdk) za detalje.
