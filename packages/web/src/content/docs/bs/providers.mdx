---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u AWMate.
---

import config from "../../../../config.mjs"
export const console = config.console

AWMate koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj AWMate konfiguraciji.
...
u `~/.local/share/awmate/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem AWMate
config.
...
AWMate Zen je lista modela koje je obezbedio AWMate tim koji su bili
testirano i potvrđeno da dobro radi sa AWMate. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa AWMate Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite AWMate i idite na [AWMate.ai/auth](https://awmate.ai/auth).
...
Radi kao i svaki drugi provajder u AWMate i potpuno je opcionalan za korištenje.

---

## AWMate Go

AWMate Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža AWMate tim i koji su testirani i verificirani da dobro rade s AWMate-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `AWMate Go` i idite na [AWMate.ai/auth](https://awmate.ai/zen).
...
Da biste koristili Amazon Bedrock s AWMate:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete AWMate:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `awmate.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u AWMate.
:::
...
Možete konfigurirati AWMate za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi AWMate ispravno radio.
   :::
...
Također možete dodati modele kroz svoju AWMate konfiguraciju.
...
da pohrani token u AWMate auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
AWMate koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate AWMate
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`awmate.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [AWMate-gitlab-auth](https://www.npmjs.com/package/awmate-gitlab-auth).

##### Konfiguracija

Prilagodite putem `awmate.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s AWMate:
...
Za korištenje Google Vertex AI s AWMate:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće AWMate.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem AWMatea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [AWMate-helicone-session](https://github.com/H2Shami/awmate-helicone-session) da automatski prijavite svaki AWMate razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki AWMate razgovor naveden kao zasebna sesija.
...
Možete konfigurirati AWMate za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati AWMate za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati AWMate za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za AWMate. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/awmate) za detalje.
:::
...
Da biste koristili Ollama Cloud s AWMate:
...
3. Kopirajte API ključ za korištenje u AWMate.
...
6. **Važno**: Prije upotrebe modela oblaka u AWMate, morate lokalno povući informacije o modelu:
...
AWMate Zen je lista testiranih i verifikovanih modela koju je obezbedio AWMate tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>AWMate Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **AWMate Zen**.

```txt
   /connect
```

3. Unesite svoj AWMate API ključ.
...
Također možete dodati dodatne modele putem vaše AWMate konfiguracije.
...
5. Također ih možete prilagoditi putem vaše AWMate konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa AWMateom:
...
Također možete prilagoditi modele kroz svoju AWMate konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s AWMate-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `awmate.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju AWMate da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `awmate auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite AWMate konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj AWMate konfiguraciji.
