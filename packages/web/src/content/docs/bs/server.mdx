---
title: Server
description: Komunicirajte s AWMate serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `AWMate serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji AWMate klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `AWMate.local` |
...
Postavite `AWMate_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `AWMate`, ili postavite `AWMate_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `AWMate serve` i za `AWMate web`.
...
Kada pokrenete `AWMate`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite AWMate server za programsku interakciju sa AWMate-om.
:::

Ova arhitektura omogucava AWMate podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `AWMate serve` da startate standalone server. Ako je AWMate TUI vec pokrenut, `AWMate serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste AWMate [IDE](/docs/ide) pluginovi.
...
AWMate server izlaže sljedece API-je.
