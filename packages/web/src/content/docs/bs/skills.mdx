...
Agent skills omogucavaju AWMate da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
AWMate pretrazuje ove lokacije:

- Konfiguracija projekta: `.awmate/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/awmate/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, AWMate ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.awmate/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/awmate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.awmate/skills/git-release/SKILL.md` ovako:
...
AWMate navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `awmate.json`:
...
**Za ugradene agente** (u `awmate.json`):
