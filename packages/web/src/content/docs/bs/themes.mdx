...
U AWMate mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, AWMate koristi nasu `AWMate` temu.
...
AWMate dolazi sa vise ugradenih tema.
...
- Zele da AWMate odgovara izgledu njihovog terminala
...
AWMate podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/awmate/themes/*.json` ili `$XDG_CONFIG_HOME/awmate/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.awmate/themes/*.json`
4. **Trenutni radni direktorij** - `./.awmate/themes/*.json`
