...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. AWMate dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u AWMate.
...
:::note
Ovaj alat je dostupan samo kada je `AWMate_EXPERIMENTAL_LSP_TOOL=true` (ili `AWMate_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz AWMate provajdera ili kada je varijabla `AWMate_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju AWMate:
