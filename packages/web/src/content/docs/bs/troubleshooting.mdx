...
Da biste otklonili probleme s AWMate, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\AWMate\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `AWMate --log-level DEBUG`.

---

## Pohrana

AWMate pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/awmate/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\AWMate`
...
AWMate Desktop pokreće lokalni AWMate server (`AWMate-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `AWMate` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/awmate/awmate.jsonc` (ili `~/.config/awmate/awmate.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/awmate/awmate.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\AWMate\awmate.jsonc`
...
AWMate također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/awmate/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\AWMate\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga AWMate mogao ponovo izgraditi.

1. Potpuno zatvorite AWMate Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/awmate`
- **Linux**: obrišite `~/.cache/awmate` (ili pokrenite `rm -rf ~/.cache/awmate`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\AWMate`

3. Ponovo pokrenite AWMate Desktop.

---

### Rješavanje problema sa vezom na serveru

AWMate Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `awmate.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `AWMate_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `AWMate_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, AWMate Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa AWMate karakteristikama.

---

### Obavještenja se ne prikazuju

AWMate Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za AWMate u postavkama vašeg OS-a, i
...
1. Zatvorite AWMate Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka AWMate Desktop aplikacije):

- `AWMate.settings.dat` (URL zadanog servera za desktop)
- `AWMate.global.dat` i `AWMate.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `AWMate models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\AWMate`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. AWMate dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\AWMate`

2. Ponovo pokrenite AWMate da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti AWMate da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
AWMate će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
