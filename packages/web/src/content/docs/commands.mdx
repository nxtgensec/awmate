...
Create `.awmate/commands/test.md`:
...
You can add custom commands through the AWMate config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your AWMate [config](/docs/config):
...
AWMate includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
