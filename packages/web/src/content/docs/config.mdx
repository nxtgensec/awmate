---
title: Config
description: Using the AWMate JSON config.
---

You can configure AWMate using a JSON config file.

---

## Format

AWMate supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="awmate.jsonc"
{
...
1. **Remote config** (from `.well-known/awmate`) - organizational defaults
2. **Global config** (`~/.config/awmate/awmate.json`) - user preferences
3. **Custom config** (`awmate_CONFIG` env var) - custom overrides
4. **Project config** (`awmate.json` in project) - project-specific settings
5. **`.awmate` directories** - agents, commands, plugins
6. **Inline config** (`awmate_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/awmate/` on macOS) - admin-controlled
...
:::note
The `.awmate` and `~/.config/awmate` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/awmate` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/awmate"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global AWMate config in `~/.config/awmate/awmate.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/awmate/tui.json`.
...
Add `awmate.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When AWMate starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `awmate_CONFIG` environment variable.

```bash
export awmate_CONFIG=/path/to/my/custom-config.json
AWMate run "Hello world"
```
...
Specify a custom config directory using the `awmate_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.awmate` directory, and should
follow the same structure.

```bash
export awmate_CONFIG_DIR=/path/to/my/config-directory
AWMate run "Hello world"
```

The custom directory is loaded after the global config and `.awmate` directories, so it **can override** their settings.
...
Drop an `awmate.json` or `awmate.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/awmate/` |
| Linux    | `/etc/awmate/`                         |
| Windows  | `%ProgramData%\AWMate`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, AWMate reads managed preferences from the `ai.awmate.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

AWMate checks these paths:

1. `/Library/Managed Preferences/<user>/ai.awmate.managed.plist`
2. `/Library/Managed Preferences/ai.awmate.managed.plist`

The plist keys map directly to `awmate.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.awmate.managed` PayloadType. The AWMate config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.awmate.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.awmate.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.awmate</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`AWMate.ai/config.json`**](https://awmate.ai/config.json).

TUI config uses [**`AWMate.ai/tui.json`**](https://awmate.ai/tui.json).
...
Use `AWMate_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `awmate.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `AWMate serve` and `AWMate web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your AWMate server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `AWMate.local`. Useful for running multiple instances on the same network.
...
If not specified, AWMate will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your AWMate config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, AWMate tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny AWMate actions on configured resources. Currently, policies can control which providers AWMate may use.
...
AWMate normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, AWMate omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/awmate/agents/` or `.awmate/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, AWMate will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`AWMate run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/awmate/commands/` or `.awmate/commands/`. [Learn more here](/docs/commands).
...
AWMate uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
AWMate will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, AWMate **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend AWMate with custom tools, hooks, and integrations.

Place plugin files in `.awmate/plugins/` or `~/.config/awmate/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="awmate.json"
{
  "$schema": "https://awmate.ai/config.json",
  "plugin": ["AWMate-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict AWMate to only use specific providers rather than disabling them one by one.
