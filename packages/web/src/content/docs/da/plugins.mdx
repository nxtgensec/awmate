---
title: Plugins
description: Skriv dine egne plugins for at udvide AWMate.
---

Plugins giver dig mulighed for at udvide AWMate ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre AWMates standardadfærd.
...
- `.awmate/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/awmate/node_modules/`.
...
1. Global konfiguration (`~/.config/awmate/awmate.json`)
2. Projektkonfiguration (`awmate.json`)
3. Global plugin-mappe (`~/.config/awmate/plugins/`)
4. Projekt plugin bibliotek (`.awmate/plugins/`)
...
AWMate kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En AWMate SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide AWMate.
...
:::note
Hvis du bruger AWMate desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå AWMate i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til AWMate:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som AWMate kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for AWMate sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://awmate.ai/docs/sdk) for detaljer.
