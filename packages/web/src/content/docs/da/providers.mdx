---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i AWMate.
---

import config from "../../../../config.mjs"
export const console = config.console

AWMate bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i AWMate-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/awmate/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i AWMate-konfigurationen.
...
AWMate Zen er en liste over modeller leveret af AWMate-teamet, som er blevet testet og verificeret til at fungere godt med AWMate. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med AWMate Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg AWMate og gå til [AWMate.ai/auth](https://awmate.ai/auth).
...
Det fungerer som alle andre udbydere i AWMate og er helt valgfrit at bruge.

---

## AWMate Go

AWMate Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af AWMate-teamet, som er testet og verificeret til at fungere godt med AWMate.

1. Kør kommandoen `/connect` i TUI, vælg `AWMate Go`, og gå til [AWMate.ai/auth](https://awmate.ai/zen).
...
Sådan bruger du Amazon Bedrock med AWMate:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører AWMate:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `awmate.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i AWMate understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere AWMate til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at AWMate skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem AWMate-konfigurationen.
...
gemme tokenet i AWMate auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
AWMate bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse AWMate
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`awmate.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [AWMate-gitlab-auth](https://www.npmjs.com/package/awmate-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `awmate.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med AWMate:
...
Sådan bruger du Google Vertex AI med AWMate:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem AWMate, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [AWMate-helicone-session](https://github.com/H2Shami/awmate-helicone-session) plugin for automatisk at logge hver AWMate-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver AWMate-samtale opført som en separat session.
...
Du kan konfigurere AWMate til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere AWMate til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere AWMate til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for AWMate. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/awmate) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med AWMate:
...
3. Kopier API-nøglen til brug i AWMate.
...
6. **Vigtigt**: Før du bruger skymodeller i AWMate, skal du hente modelinformationen lokalt:
...
AWMate Zen er en liste over testede og verificerede modeller leveret af AWMate-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>AWMate Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **AWMate Zen**.

   ```txt
   /connect
   ```

3. Indtast AWMate API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem AWMate-konfigurationen.
...
5. Du kan også tilpasse dem gennem AWMate-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med AWMate:
...
Du kan også tilpasse modeller gennem AWMate-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med AWMate. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `awmate.json`-filen i projektkataloget:
...
`limit`-felterne lader AWMate forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `awmate auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek AWMate-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i AWMate-konfigurationen.
