---
title: Server
description: Interager med AWMate-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `AWMate serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en AWMate-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `AWMate.local` |
...
Indstil `AWMate_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `AWMate`, eller indstil `AWMate_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `AWMate serve` og `AWMate web`.
...
Når du kører `AWMate` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug AWMate-serveren til at interagere med AWMate programmatisk.
:::

Denne arkitektur lader AWMate understøtte flere klienter og lader dig interagere med AWMate programmatisk.

Du kan køre `AWMate serve` for at starte en selvstændig server. Hvis du har
AWMate TUI kørende, vil `AWMate serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af AWMate [IDE](/docs/ide) plugins.
...
AWMate-serveren viser følgende API'er.
