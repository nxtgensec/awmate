...
Agentferdigheter lar AWMate oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
AWMate søker etter disse stedene:

- Prosjektkonfigurasjon: `.awmate/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/awmate/skills/<name>/SKILL.md`
...
For prosjektlokale stier går AWMate opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.awmate/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/awmate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.awmate/skills/git-release/SKILL.md` slik:
...
AWMate viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `awmate.json`:
...
**For innebygde agenter** (i `awmate.json`):
