...
Med AWMate kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger AWMate vårt eget `AWMate`-tema.
...
AWMate kommer med flere innebygde temaer.
...
- Vil at AWMate skal matche terminalens utseende
...
AWMate støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/awmate/themes/*.json` eller `$XDG_CONFIG_HOME/awmate/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.awmate/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.awmate/themes/*.json`
