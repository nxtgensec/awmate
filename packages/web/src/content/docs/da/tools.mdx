...
Her er alle de innebygde verktøyene som er tilgængelige i AWMate.
...
:::note
Dette verktøyet er bare tilgængelig når `AWMate_EXPERIMENTAL_LSP_TOOL=true` (eller `AWMate_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger AWMate-leverandøren eller når miljøvariabelen `AWMate_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
