...
For at fejlfinde problemer med AWMate, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\AWMate\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `AWMate --log-level DEBUG`.

---

## Lagring

AWMate gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/awmate/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\AWMate`
...
AWMate Desktop kører en lokal AWMate-server (`AWMate-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `AWMate`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/awmate/awmate.jsonc` (eller `~/.config/awmate/awmate.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/awmate/awmate.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\AWMate\awmate.jsonc`
...
AWMate kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/awmate/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\AWMate\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så AWMate kan genopbygge den.

1. Afslut AWMate Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/awmate`
- **Linux**: slet `~/.cache/awmate` (eller kør `rm -rf ~/.cache/awmate`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\AWMate`

3. Start AWMate Desktop på ny.

---

### Løs problemer med serverforbindelse

AWMate Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `awmate.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `AWMate_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `AWMate_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver AWMate Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i AWMate.

---

### Meddelelser vises ikke

AWMate Desktop viser kun systemvarsler når:

- varsler er aktiveret for AWMate i dine OS-indstillinger, og
...
1. Afslut AWMate Desktop.
2. Find og slet disse filer (de findes i AWMate Desktop-appens datamappe):

- `AWMate.settings.dat` (skrivebordsstandardserver URL)
- `AWMate.global.dat` og `AWMate.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med AWMate:
...
For at finde ud af hvilke modeller du har adgang til, kør `AWMate models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\AWMate`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. AWMate installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\AWMate`

2. Start AWMate på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge AWMate til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
AWMate vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
