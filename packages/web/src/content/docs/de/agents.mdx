...
Es gibt zwei Arten von Agenten in AWMate; Primäragenten und Subagenten.
...
AWMate verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `awmate.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet AWMate modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale AWMate-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer AWMate-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [AWMate Zen](/docs/zen) verwenden, würden Sie `awmate/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `AWMate models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/awmate).
:::
