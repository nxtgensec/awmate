---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um AWMate zu erweitern.
---

Mit Plugins können Sie AWMate erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von AWMate zu ändern.
...
- `.awmate/plugins/` – Plugins auf Projektebene
- `~/.config/awmate/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/awmate/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/awmate/awmate.json`)
2. Projektkonfiguration (`awmate.json`)
3. Globales Plugin-Verzeichnis (`~/.config/awmate/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.awmate/plugins/`)
...
AWMate führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein AWMate-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie AWMate erweitern können.
...
:::note
Wenn Sie die AWMate-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass AWMate `.env`-Dateien liest:
...
Plugins können AWMate auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das AWMate aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für AWMate zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://awmate.ai/docs/sdk).
