---
title: Regeln
description: Lege eigene Anweisungen fuer AWMate fest.
---

Du kannst AWMate ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in AWMate den Befehl `/init` aus.
...
So kann AWMate sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht AWMate sie zu erweitern.
...
AWMate kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/awmate/AGENTS.md` speichern.
Diese gelten in allen AWMate-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt AWMate dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/awmate/AGENTS.md` existiert)
...
Beim Start sucht AWMate in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/awmate/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `awmate.json` oder in `~/.config/awmate/awmate.json` angeben.
...
AWMate parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit awmate.json

Empfohlen ist das `instructions`-Feld in der `awmate.json`:
...
Du kannst AWMate explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `awmate.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
