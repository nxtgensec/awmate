---
title: Server
description: Nutze den AWMate-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `AWMate serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein AWMate-Client nutzen kann.
...
Setze `AWMate_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `AWMate`, kann aber mit `AWMate_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `AWMate serve` und `AWMate web`.
...
Wenn du `AWMate` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den AWMate-Server, um AWMate programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `AWMate serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `AWMate serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die AWMate-[IDE](/docs/ide)-Plugins.
...
Der AWMate-Server stellt folgende APIs bereit.
