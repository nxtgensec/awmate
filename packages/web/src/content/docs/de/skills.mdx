...
Agent Skills erlauben AWMate, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
AWMate sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft AWMate vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.awmate/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/awmate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.awmate/skills/git-release/SKILL.md` zum Beispiel so:
...
AWMate listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `awmate.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `awmate.json`):
