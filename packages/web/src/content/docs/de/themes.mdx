...
Mit AWMate kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet AWMate das `AWMate`-Theme.
...
AWMate liefert mehrere eingebaute Themes mit.
...
- AWMate optisch ans Terminal anpassen wollen
...
AWMate unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/awmate/themes/*.json` oder `$XDG_CONFIG_HOME/awmate/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.awmate/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.awmate/themes/*.json`
