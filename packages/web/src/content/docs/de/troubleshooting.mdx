...
Wenn AWMate Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\AWMate\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `AWMate --log-level DEBUG`.

---

## Speicher

AWMate speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/awmate/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\AWMate`
...
AWMate Desktop startet im Hintergrund einen lokalen AWMate-Server (`AWMate-cli`-Sidecar).
...
- Nur macOS: `AWMate`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/awmate/awmate.jsonc` (oder `~/.config/awmate/awmate.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/awmate/awmate.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\AWMate\awmate.jsonc`
...
AWMate kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/awmate/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\AWMate\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit AWMate ihn neu aufbauen kann.

1. Beende AWMate Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/awmate`
- **Linux**: loesche `~/.cache/awmate` (oder fuehre aus: `rm -rf ~/.cache/awmate`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\AWMate`

3. Starte AWMate Desktop neu.

---

### Server-Verbindungsprobleme beheben

AWMate Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `awmate.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `AWMate_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `AWMate_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt AWMate Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit AWMate-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

AWMate Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer AWMate in den OS-Einstellungen aktiviert sind, und
...
1. Beende AWMate Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von AWMate Desktop):

- `AWMate.settings.dat` (desktop default server URL)
- `AWMate.global.dat` und `AWMate.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit AWMate hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `AWMate models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\AWMate`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. AWMate installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\AWMate`

2. Starte AWMate neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt AWMate, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
AWMate erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
