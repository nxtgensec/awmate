...
description: Cree herramientas que LLM pueda llamar en AWMate.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de AWMate como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.awmate/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/awmate/tools/`.
...
```ts title=".awmate/tools/database.ts" {1}
import { tool } from "@AWMate-ai/plugin"
...
```ts title=".awmate/tools/project.ts" {8}
import { tool } from "@AWMate-ai/plugin"
...
```python title=".awmate/tools/add.py"
import sys
...
```ts title=".awmate/tools/python-add.ts" {10}
import { tool } from "@AWMate-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".awmate/tools/add.py")
