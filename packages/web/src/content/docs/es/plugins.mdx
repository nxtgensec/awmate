---
title: Complementos
description: Escriba sus propios complementos para extender AWMate.
---

Los complementos le permiten extender AWMate al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de AWMate.
...
- `.awmate/plugins/` - Complementos a nivel de proyecto
- `~/.config/awmate/plugins/` - Complementos globales
...
```json title="awmate.json"
{
  ...
  "plugin": ["AWMate-helicone-session", "AWMate-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/awmate/node_modules/`.
...
1. Configuración global (`~/.config/awmate/awmate.json`)
2. Configuración del proyecto (`awmate.json`)
3. Directorio global de complementos (`~/.config/awmate/plugins/`)
4. Directorio de complementos del proyecto (`.awmate/plugins/`)
...
```json title=".awmate/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
AWMate ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".awmate/plugins/my-plugin.ts"
...
```js title=".awmate/plugins/example.js"
...
- `client`: Un cliente SDK AWMate para interactuar con la IA.
...
import type { Plugin } from "@AWMate-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar AWMate.
...
```js title=".awmate/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "AWMate"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio AWMate, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que AWMate lea archivos `.env`:

```javascript title=".awmate/plugins/env-protection.js"
...
```javascript title=".awmate/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a AWMate:

```ts title=".awmate/plugins/custom-tools.ts"
import { type Plugin, tool } from "@AWMate-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que AWMate puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para AWMate junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://awmate.ai/docs/sdk) para obtener más detalles.
...
```ts title=".awmate/plugins/compaction.ts"
import type { Plugin } from "@AWMate-ai/plugin"
...
```ts title=".awmate/plugins/custom-compaction.ts"
import type { Plugin } from "@AWMate-ai/plugin"
