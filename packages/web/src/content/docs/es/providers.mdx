---
title: Proveedores
description: Usando cualquier proveedor LLM en AWMate.
---

import config from "../../../../config.mjs"
export const console = config.console

AWMate usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración AWMate.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu AWMate
configuración.
...
AWMate Zen es una lista de modelos proporcionados por el equipo AWMate que han sido
probado y verificado para funcionar bien con AWMate. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con AWMate Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione AWMate y diríjase a [AWMate.ai/auth](https://awmate.ai/auth).
...
Funciona como cualquier otro proveedor en AWMate y su uso es completamente opcional.

---

## AWMate Go

AWMate Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de AWMate que han sido
probado y verificado para funcionar bien con AWMate.

1. Ejecute el comando `/connect` en TUI, seleccione `AWMate Go` y diríjase a [AWMate.ai/auth](https://awmate.ai/zen).
...
Para usar Amazon Bedrock con AWMate:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta AWMate:
   ...
   Para una configuración persistente o específica del proyecto, utilice `awmate.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en AWMate no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar AWMate para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que AWMate funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración AWMate.
...
para almacenar el token en el almacenamiento de autenticación AWMate.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
AWMate utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear AWMate
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `awmate.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [AWMate-gitlab-auth](https://www.npmjs.com/package/awmate-gitlab-auth).

##### Configuración

Personalizar a través de `awmate.json`:
...
Para utilizar su suscripción GitHub Copilot con AWMate:
...
Para utilizar Google Vertex AI con AWMate:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta AWMate.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de AWMate, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [AWMate-helicone-session](https://github.com/H2Shami/awmate-helicone-session) para registrar automáticamente cada conversación AWMate como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación AWMate enumerada como una sesión separada.
...
Puede configurar AWMate para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar AWMate para usar modelos locales a través de LM Studio.
...
Puedes configurar AWMate para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para AWMate. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/awmate) para obtener más detalles.
:::
...
Para usar Ollama Cloud con AWMate:
...
3. Copie la clave API para usarla en AWMate.
...
6. **Importante**: Antes de usar modelos en la nube en AWMate, debe obtener la información del modelo localmente:
...
AWMate Zen es una lista de modelos probados y verificados proporcionada por el equipo AWMate. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>AWMate Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **AWMate Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave AWMate API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración AWMate.
...
5. También puedes personalizarlos a través de tu configuración AWMate. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con AWMate:
...
También puede personalizar modelos a través de su configuración AWMate. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con AWMate. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `awmate.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a AWMate comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `awmate auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración AWMate y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración AWMate.
