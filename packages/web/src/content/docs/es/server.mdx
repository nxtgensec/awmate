---
title: Servidor
description: Interactuar con el servidor AWMate a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `AWMate serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente AWMate puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `AWMate.local` |
...
Configure `AWMate_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `AWMate`, o configure `AWMate_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `AWMate serve` como a `AWMate web`.
...
Cuando ejecuta `AWMate`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor AWMate para interactuar con AWMate mediante programación.
:::

Esta arquitectura permite que AWMate admita múltiples clientes y le permite interactuar con AWMate mediante programación.

Puede ejecutar `AWMate serve` para iniciar un servidor independiente. Si tienes el
AWMate TUI ejecutándose, `AWMate serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos AWMate [IDE](/docs/ide).
...
El servidor AWMate expone las siguientes API.
