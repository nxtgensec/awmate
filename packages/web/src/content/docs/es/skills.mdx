...
Las habilidades del agente permiten a AWMate descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
AWMate busca estas ubicaciones:

- Configuración del proyecto: `.awmate/skills/<name>/SKILL.md`
- Configuración global: `~/.config/awmate/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, AWMate sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.awmate/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/awmate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.awmate/skills/git-release/SKILL.md` así:
...
AWMate enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `awmate.json`:
...
**Para agentes integrados** (en `awmate.json`):
