...
Con AWMate puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, AWMate usa nuestro propio tema `AWMate`.
...
AWMate viene con varios temas integrados.
...
- Quiere que AWMate coincida con la apariencia de su terminal
...
AWMate admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/awmate/themes/*.json` o `$XDG_CONFIG_HOME/awmate/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.awmate/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.awmate/themes/*.json`
