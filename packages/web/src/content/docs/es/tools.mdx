...
Las herramientas permiten que LLM realice acciones en su código base. AWMate viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en AWMate.
...
:::note
Esta herramienta solo está disponible cuando `AWMate_EXPERIMENTAL_LSP_TOOL=true` (o `AWMate_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor AWMate o cuando la variable de entorno `AWMate_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar AWMate:

```bash
AWMate_ENABLE_EXA=1 AWMate
```
