...
Para depurar problemas con AWMate, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/awmate/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\AWMate\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `AWMate --log-level DEBUG`.

---

## Almacenamiento

AWMate almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/awmate/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\AWMate`
...
AWMate Desktop ejecuta un servidor AWMate local (el sidecar `AWMate-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `AWMate` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/awmate/awmate.jsonc` (o `~/.config/awmate/awmate.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/awmate/awmate.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\AWMate\awmate.jsonc`
...
AWMate también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/awmate/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\AWMate\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.awmate/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que AWMate pueda reconstruirlo.

1. Salga de AWMate Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/awmate`
- **Linux**: eliminar `~/.cache/awmate` (o ejecutar `rm -rf ~/.cache/awmate`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\AWMate`

3. Reinicie el escritorio AWMate.

---

### Solucionar problemas de conexión del servidor

AWMate Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `awmate.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `AWMate_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `AWMate_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio AWMate requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de AWMate.

---

### Notificaciones que no se muestran

AWMate Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para AWMate en la configuración de su sistema operativo, y
...
1. Salga del escritorio AWMate.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio AWMate):

- `AWMate.settings.dat` (URL del servidor predeterminado de escritorio)
- `AWMate.global.dat` y `AWMate.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/awmate/issues**](https://github.com/anomalyco/awmate/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**AWMate.ai/discord**](https://awmate.ai/discord)
...
3. Asegúrese de tener la última versión con `AWMate upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `AWMate models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/awmate
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\AWMate`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. AWMate instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\AWMate`

2. Reinicie AWMate para reinstalar los paquetes del proveedor más recientes.

Esto obligará a AWMate a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
AWMate detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
