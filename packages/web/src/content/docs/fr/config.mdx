---
title: Configuration
description: Utilisation de la configuration JSON AWMate.
---

Vous pouvez configurer AWMate à l'aide d'un fichier de configuration JSON.

---

## Format

AWMate prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/awmate`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/awmate/awmate.json`) - préférences utilisateur
3. **Configuration personnalisée** (`awmate_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`awmate.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.awmate`** - agents, commandes, plugins
6. **Configuration en ligne** (`awmate_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.awmate` et `~/.config/awmate` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/awmate`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale AWMate dans `~/.config/awmate/awmate.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/awmate/tui.json`.
...
Ajoutez `awmate.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque AWMate démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `awmate_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `awmate_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.awmate`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.awmate`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`AWMate.ai/config.json`**](https://awmate.ai/config.json).

La configuration TUI utilise [**`AWMate.ai/tui.json`**](https://awmate.ai/tui.json).
...
Utilisez `AWMate_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `awmate.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `AWMate serve` et `AWMate web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur AWMate.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `AWMate.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration AWMate via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, AWMate essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/awmate/agents/` ou `.awmate/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, AWMate reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`AWMate run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/awmate/commands/` ou `.awmate/commands/`. [En savoir plus ici](/docs/commands).
...
AWMate téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, AWMate **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent AWMate avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.awmate/plugins/` ou `~/.config/awmate/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre AWMate à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
