...
Vous pouvez ajouter des outils externes à AWMate à l'aide du _Model Context Protocol_ ou MCP. AWMate prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [AWMate Config](https://awmate.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/awmate`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
AWMate gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, AWMate :
...
Si le serveur nécessite une authentification, AWMate vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `awmate mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, AWMate stockera les jetons en toute sécurité dans `~/.local/share/awmate/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans AWMate, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration AWMate comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter AWMate à votre compte Sentry.
