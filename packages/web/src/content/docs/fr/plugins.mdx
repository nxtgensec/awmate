---
title: Plugins
description: Écrivez vos propres plugins pour étendre AWMate.
---

Les plugins vous permettent d'étendre AWMate en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de AWMate.
...
- `.awmate/plugins/` - Plugins au niveau du projet
- `~/.config/awmate/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/awmate/node_modules/`.
...
1. Configuration globale (`~/.config/awmate/awmate.json`)
2. Configuration du projet (`awmate.json`)
3. Répertoire global des plugins (`~/.config/awmate/plugins/`)
4. Répertoire des plugins du projet (`.awmate/plugins/`)
...
AWMate exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK AWMate pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre AWMate.
...
:::note
Si vous utilisez l'application de bureau AWMate, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez AWMate de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à AWMate :
...
L'assistant `tool` crée un outil personnalisé que AWMate peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour AWMate aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://awmate.ai/docs/sdk) pour plus de détails.
