---
title: SDK
description: Client JS de type sécurisé pour le serveur AWMate.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK AWMate JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler AWMate par programme.
...
Créez une instance de AWMate :

```javascript
import { createAWMate } from "@AWMate-ai/sdk"

const { client } = await createAWMate()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `awmate.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createAWMate } from "@AWMate-ai/sdk"

const AWMate = await createAWMate({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${AWMate.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de AWMate, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createAWMateClient } from "@AWMate-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@AWMate-ai/sdk"
```
