---
title: Serveur
description: Interagissez avec le serveur AWMate via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `AWMate serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client AWMate peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `AWMate.local` |
...
Définissez `AWMate_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `AWMate`, ou définissez `AWMate_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `AWMate serve` et à `AWMate web`.
...
Lorsque vous exécutez `AWMate`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur AWMate pour interagir avec AWMate par programmation.
:::

Cette architecture permet à AWMate de prendre en charge plusieurs clients et vous permet d'interagir avec AWMate par programme.

Vous pouvez exécuter `AWMate serve` pour démarrer un serveur autonome. Si vous avez le
AWMate TUI en cours d'exécution, `AWMate serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins AWMate [IDE](/docs/ide).
...
Le serveur AWMate expose les API suivantes.
