...
Avec AWMate, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, AWMate utilise notre propre thème `AWMate`.
...
AWMate est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que AWMate corresponde à l'apparence de leur terminal
...
AWMate prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/awmate/themes/*.json` ou `$XDG_CONFIG_HOME/awmate/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.awmate/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.awmate/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/awmate/themes
vim ~/.config/awmate/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .awmate/themes
vim .awmate/themes/my-theme.json
```
...
{
  "$schema": "https://awmate.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
