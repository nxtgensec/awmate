...
Puoi aggiungere comandi personalizzati tramite la config di AWMate o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di AWMate:
...
- Globale: `~/.config/awmate/commands/`
- Per progetto: `.awmate/commands/`
...
AWMate include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
