...
Puoi aggiungere strumenti esterni a AWMate usando il _Model Context Protocol_, o MCP. AWMate supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [AWMate Config](https://awmate.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/awmate`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
AWMate gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, AWMate:
...
Se il server richiede autenticazione, AWMate ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `awmate mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, AWMate memorizzera' i token in modo sicuro in `~/.local/share/awmate/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in AWMate insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione AWMate come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare AWMate al tuo account Sentry.
