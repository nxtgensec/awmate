---
title: Plugin
description: Scrivi plugin per estendere AWMate.
---

I plugin ti permettono di estendere AWMate agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di AWMate.
...
- `.awmate/plugins/` - plugin a livello progetto
- `~/.config/awmate/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/awmate/node_modules/`.
...
1. Config globale (`~/.config/awmate/awmate.json`)
2. Config di progetto (`awmate.json`)
3. Directory plugin globale (`~/.config/awmate/plugins/`)
4. Directory plugin di progetto (`.awmate/plugins/`)
...
AWMate esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di AWMate per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere AWMate.
...
:::note
Se usi l'app desktop di AWMate, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a AWMate di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a AWMate:
...
L'helper `tool` crea uno strumento personalizzato che AWMate può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in AWMate insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://awmate.ai/docs/sdk) per i dettagli.
