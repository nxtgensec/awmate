---
title: Server
description: Interagisci con il server AWMate via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `AWMate serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client AWMate.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `AWMate.local` |
...
Imposta `AWMate_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `AWMate`, oppure imposta `AWMate_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `AWMate serve` sia per `AWMate web`.
...
Quando esegui `AWMate` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server AWMate per interagire con AWMate in modo programmatico.
:::

Questa architettura permette a AWMate di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `AWMate serve` per avviare un server standalone. Se la TUI di AWMate e gia in esecuzione, `AWMate serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di AWMate.
...
Il server AWMate espone le seguenti API.
