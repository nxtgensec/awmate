...
Le skill degli agenti permettono a AWMate di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
AWMate cerca in queste posizioni:

- Config di progetto: `.awmate/skills/<name>/SKILL.md`
- Config globale: `~/.config/awmate/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, AWMate risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.awmate/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/awmate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.awmate/skills/git-release/SKILL.md` cosi':
...
AWMate elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `awmate.json`:
...
**Per agenti integrati** (in `awmate.json`):
