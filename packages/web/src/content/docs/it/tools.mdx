...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. AWMate include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in AWMate.
...
:::note
Questo strumento e' disponibile solo quando `AWMate_EXPERIMENTAL_LSP_TOOL=true` (oppure `AWMate_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider AWMate o quando la variabile d'ambiente `AWMate_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di AWMate:
