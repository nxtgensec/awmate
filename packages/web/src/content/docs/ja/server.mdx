---
title: サーバー
description: HTTP 経由で AWMate サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`AWMate serve` コマンドは、AWMate クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `AWMate.local` |
...
HTTP 基本認証でサーバーを保護するには、`AWMate_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `AWMate` になるか、`AWMate_SERVER_USERNAME` を設定してオーバーライドします。これは、`AWMate serve` と `AWMate web` の両方に当てはまります。
...
`AWMate` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
AWMate サーバーを使用して、プログラムで AWMate と対話します。
:::

`AWMate serve` を実行してスタンドアロンサーバーを起動できます。すでに AWMate TUI を実行している場合でも、`AWMate serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、AWMate [IDE](/docs/ide) プラグインによって使用されます。
