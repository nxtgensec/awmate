...
You can add external tools to AWMate using the _Model Context Protocol_, or MCP. AWMate supports both local and remote servers.
...
You can define MCP servers in your [AWMate Config](https://awmate.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/awmate` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
AWMate automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, AWMate will:
...
If the server requires authentication, AWMate will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `awmate mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, AWMate will store the tokens securely in `~/.local/share/awmate/mcp-auth.json`.
...
Your MCPs are available as tools in AWMate, alongside built-in tools. So you can manage them through the AWMate config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect AWMate to your Sentry account.
