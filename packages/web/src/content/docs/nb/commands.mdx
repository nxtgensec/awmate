...
Opprett `.awmate/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom AWMate-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i AWMate [config](/docs/config):
...
AWMate inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
