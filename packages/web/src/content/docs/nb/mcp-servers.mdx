...
Du kan legge til eksterne verktøy i AWMate ved å bruke _Model Context Protocol_ eller MCP. AWMate støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [AWMate Config](https://awmate.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="awmate.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/awmate`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
AWMate håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil AWMate:
...
Hvis serveren krever autentisering, vil AWMate be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `awmate mcp auth <server-name>`.
...
```bash
awmate mcp auth my-oauth-server
```
...
```bash
AWMate mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil AWMate lagre tokens sikkert i `~/.local/share/awmate/mcp-auth.json`.
...
AWMate mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i AWMate, sammen med innebygde verktøy. Så du kan administrere dem gjennom AWMate-konfigurasjonen som alle andre verktøy.
...
```json title="awmate.json" {11, 14-18}
{
  "$schema": "https://awmate.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
awmate mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble AWMate til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver AWMate-samtale oppført som en separat økt.
