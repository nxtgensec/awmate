...
description: Skriv dine egne programtillegg for å utvide AWMate.
---

Plugins lar deg utvide AWMate ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til AWMate.
...
- `.awmate/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/awmate/node_modules/`.
...
1. Global konfigurasjon (`~/.config/awmate/awmate.json`)
2. Prosjektkonfigurasjon (`awmate.json`)
3. Global plugin-katalog (`~/.config/awmate/plugins/`)
4. Prosjektpluginkatalog (`.awmate/plugins/`)
...
AWMate kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En AWMate SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide AWMate.
...
:::note
Hvis du bruker AWMate-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre AWMate fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til AWMate:
...
`tool`-hjelperen lager et tilpasset verktøy som AWMate kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for AWMate sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://awmate.ai/docs/sdk) for detaljer.
