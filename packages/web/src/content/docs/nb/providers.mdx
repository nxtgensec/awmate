---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i AWMate.
---

import config from "../../../../config.mjs"
export const console = config.console

AWMate bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i AWMate-konfigurasjonen.
...
i `~/.local/share/awmate/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i AWMate
konfig.
...
AWMate Zen er en liste over modeller levert av AWMate-teamet som har vært
testet og verifisert for å fungere godt med AWMate. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med AWMate Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `AWMate Zen` og gå til [AWMate.ai/auth](https://awmate.ai/zen).
...
Det fungerer som alle andre leverandører i AWMate og er helt valgfritt å bruke.

---

## AWMate Go

AWMate Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av AWMate-teamet som har vært
testet og verifisert for å fungere godt med AWMate.

1. Kjør kommandoen `/connect` i TUI, velg `AWMate Go`, og gå til [AWMate.ai/auth](https://awmate.ai/zen).
...
Slik bruker du Amazon Bedrock med AWMate:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører AWMate:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `awmate.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i AWMate støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere AWMate til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at AWMate skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom AWMate-konfigurasjonen.
...
for å lagre token i AWMate auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
AWMate bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse AWMate
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`awmate.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [AWMate-gitlab-auth](https://www.npmjs.com/package/awmate-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `awmate.json`:
...
Slik bruker du GitHub Copilot-abonnementet med AWMate:
...
Slik bruker du Google Vertex AI med AWMate:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører AWMate.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom AWMate, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [AWMate-helicone-session](https://github.com/H2Shami/awmate-helicone-session) plugin for automatisk å logge hver AWMate-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver AWMate-samtale oppført som en separat økt.
...
Du kan konfigurere AWMate for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere AWMate for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere AWMate for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for AWMate. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/awmate) for detaljer.
:::
...
Slik bruker du Ollama Cloud med AWMate:
...
3. Kopier API-nøkkelen for bruk i AWMate.
...
6. **Viktig**: Før du bruker skymodeller i AWMate, må du hente modellinformasjonen lokalt:
...
AWMate Zen er en liste over testede og verifiserte modeller levert av AWMate-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>AWMate Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **AWMate Zen**.

   ```txt
   /connect
   ```

3. Skriv inn AWMate API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom AWMate-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom AWMate-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med AWMate:
...
Du kan også tilpasse modeller gjennom AWMate-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med AWMate. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `awmate.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar AWMate forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `awmate auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk AWMate-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i AWMate-konfigurasjonen.
