...
Możesz dodać dodatkowe informacje do AWMate za pomocą _Model Context Protocol_, w skrócie MCP. AWMate obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [AWMate Config](https://awmate.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/awmate`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
AWMate automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, AWMate:
...
Jeśli serwer wymaga uwierzytelnienia, AWMate poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `awmate mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji AWMate bezpieczne przechowa tokeny w `~/.local/share/awmate/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w AWMate, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie AWMate, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć AWMate z kontem Sentry.
