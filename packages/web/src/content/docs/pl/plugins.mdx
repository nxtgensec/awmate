---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć AWMate.
---

Wtyczki umożliwiające wyłączenie AWMate poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania AWMate.
...
- `.awmate/plugins/` - Wtyczki na poziomie projektu
- `~/.config/awmate/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/awmate/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/awmate/awmate.json`)
2. Konfiguracja projektu (`awmate.json`)
3. Globalny katalog wtyczek (`~/.config/awmate/plugins/`)
4. Katalog wtyczek projektu (`.awmate/plugins/`)
...
AWMate uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z AWMate do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do AWMate.
...
:::note
Jeśli korzystasz z aplikacji komputerowej AWMate, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj AWMate czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do AWMate:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod AWMate. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://awmate.ai/docs/sdk).
