---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w AWMate.
---

import config from "../../../../config.mjs"
export const console = config.console

AWMate używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji AWMate.
...
w `~/.local/share/awmate/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim AWMate
config.
...
AWMate Zen to lista modeli dostarczonych przez zespół AWMate, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z AWMate. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od AWMate Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `AWMate Zen` i przejdź do [AWMate.ai/auth](https://awmate.ai/zen).
...
Działa jak każdy inny dostawca w AWMate i jest całkowicie opcjonalny w użyciu.

---

## AWMate Go

AWMate Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół AWMate, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z AWMate.

1. Uruchom polecenie `/connect` w TUI, wybierz `AWMate Go` i przejdź do [AWMate.ai/auth](https://awmate.ai/zen).
...
Aby używać Amazon Bedrock z AWMate:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania AWMate:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `awmate.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w AWMate nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować AWMate do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod AWMate działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji AWMate.
...
AWMate używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować AWMate
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`awmate.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [AWMate-gitlab-auth](https://www.npmjs.com/package/awmate-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem AWMate:
...
Aby używać Google Vertex AI z AWMate:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania AWMate.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą AWMate, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [AWMate-helicone-session](https://github.com/H2Shami/awmate-helicone-session), aby automatycznie rejestrować każdą konwersację AWMate jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja AWMate będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować AWMate tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować AWMate tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować AWMate tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem AWMate. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/awmate).
:::
...
Aby korzystać z Ollama Cloud z AWMate:
...
3. Skopiuj klucz API do użycia w AWMate.
...
6. **Ważne**: Przed użyciem modeli chmurowych w AWMate musisz pobrać informacje o modelu lokalnie:
...
AWMate Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół AWMate. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>AWMate Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **AWMate Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API AWMate.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji AWMate.
...
5. Możesz je także dostosować za pomocą konfiguracji AWMate. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z AWMate:
...
Możesz także dostosować modele za pomocą konfiguracji AWMate. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z AWMate. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `awmate.json` w katalogu projektu:
...
Pola `limit` pozwalają AWMate zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `awmate auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację AWMate i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji AWMate.
