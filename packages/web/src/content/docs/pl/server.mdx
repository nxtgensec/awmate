---
title: Serwer
description: Interakcja z serwerem AWMate za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `AWMate serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient AWMate.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `AWMate.local` |
...
Ustaw `AWMate_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `AWMate` lub ustaw `AWMate_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `AWMate serve`, jak i `AWMate web`.
...
Kiedy uruchomisz `AWMate`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera AWMate do programowej interakcji z AWMate.
:::

Ta architektura umożliwia obsługę wielu klientów przez AWMate i programową interakcję z kodem otwartym.

Możesz uruchomić `AWMate serve`, aby uruchomić samodzielny serwer. Jeśli masz
AWMate TUI działa, `AWMate serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki AWMate [IDE](/docs/ide).
...
Serwer AWMate udostępnia następujące interfejsy API.
