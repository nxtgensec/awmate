...
Umiejętności agenta pozwalają AWMate odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
AWMate przeszukuje te lokalizacje:

- Project config: `.awmate/skills/<name>/SKILL.md`
- Global config: `~/.config/awmate/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu AWMate przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.awmate/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/awmate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.awmate/skills/git-release/SKILL.md` w ten sposób:
...
AWMate wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `awmate.json`:
...
**Dla agentów wbudowanych** (w `awmate.json`):
