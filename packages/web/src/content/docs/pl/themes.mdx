...
Dzięki AWMate możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie AWMate używa naszego własnego motywu `AWMate`.
...
AWMate ma kilka wbudowanych motywów.
...
- Chcą, aby AWMate pasował do wyglądu ich terminala
...
AWMate obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/awmate/themes/*.json` lub `$XDG_CONFIG_HOME/awmate/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.awmate/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.awmate/themes/*.json`
