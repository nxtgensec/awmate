...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. AWMate posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w AWMate.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `AWMate_EXPERIMENTAL_LSP_TOOL=true` (lub `AWMate_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy AWMate lub gdy zmienna środowiskowa `AWMate_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu AWMate:
