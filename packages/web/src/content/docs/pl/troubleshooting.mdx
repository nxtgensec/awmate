...
Aby debugować problemy z AWMate, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\AWMate\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `AWMate --log-level DEBUG`.

---

## Przechowywanie danych

AWMate przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/awmate/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\AWMate`
...
AWMate Desktop uruchamia w tle lokalny serwer AWMate (sidecar `AWMate-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `AWMate` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/awmate/awmate.jsonc` (lub `~/.config/awmate/awmate.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/awmate/awmate.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\AWMate\awmate.jsonc`
...
AWMate może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/awmate/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\AWMate\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby AWMate mógł ją odbudować.

1. Całkowicie zamknij AWMate Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/awmate`
- **Linux**: usuń `~/.cache/awmate` (lub uruchom `rm -rf ~/.cache/awmate`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\AWMate`

3. Uruchom ponownie AWMate Desktop.

---

### Napraw problemy z połączeniem z serwerem

AWMate Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `awmate.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `AWMate_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `AWMate_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows AWMate Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami AWMate.

---

### Brak powiadomień

AWMate Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla AWMate w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie AWMate Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji AWMate Desktop):

- `AWMate.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `AWMate.global.dat` i `AWMate.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `AWMate upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `AWMate models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\AWMate`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. AWMate dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\AWMate`

2. Uruchom ponownie kod AWMate, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to AWMate do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
AWMate wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
