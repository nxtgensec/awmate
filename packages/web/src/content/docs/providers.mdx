---
title: Providers
description: Using any LLM provider in AWMate.
---

import config from "../../../config.mjs"
export const console = config.console

AWMate uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your AWMate config.
...
in `~/.local/share/awmate/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your AWMate
config.
...
AWMate Zen is a list of models provided by the AWMate team that have been
tested and verified to work well with AWMate. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with AWMate Zen.
:::

1. Run the `/connect` command in the TUI, select `AWMate Zen`, and head to [AWMate.ai/auth](https://awmate.ai/zen).
...
It works like any other provider in AWMate and is completely optional to use.

---

## AWMate Go

AWMate Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the AWMate team that have been
tested and verified to work well with AWMate.

1. Run the `/connect` command in the TUI, select `AWMate Go`, and head to [AWMate.ai/auth](https://awmate.ai/zen).
...
To use Amazon Bedrock with AWMate:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running AWMate:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY AWMate

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile AWMate

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX AWMate
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `awmate.json`:
...
AWMate. Anthropic explicitly prohibits this.

Previous versions of AWMate came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in AWMate with zero setup:
...
You can configure AWMate to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="awmate.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://awmate.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for AWMate to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX AWMate
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX AWMate
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your AWMate config.
...
AWMate supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; AWMate uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize AWMate. Sign in and approve.

   :::note
   AWMate requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, AWMate surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — AWMate forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in AWMate is experimental. Features, configuration, and
behavior may change in future releases.
:::

AWMate integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in AWMate auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
AWMate uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock AWMate
to only use your own GitLab-hosted instance, add the following to your
`awmate.json` file. It is also recommended to disable session sharing.
...
More documentation on [AWMate-gitlab-auth](https://www.npmjs.com/package/awmate-gitlab-auth) homepage.

##### Configuration

Customize through `awmate.json`:
...
When a `duo-workflow-*` model is selected, AWMate will:
...
4. Route tool execution requests through AWMate's permission-gated tool system
...
To use your GitHub Copilot subscription with AWMate:
...
To use Google Vertex AI with AWMate:
...
2. Set the required environment variables:
   ...
   Set them while running AWMate.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id AWMate
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through AWMate, you can always configure it yourself.
...
```jsonc title="~/.config/awmate/awmate.jsonc"
{
  "$schema": "https://awmate.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [AWMate-helicone-session](https://github.com/H2Shami/awmate-helicone-session) plugin to automatically log each AWMate conversation as a session in Helicone.

```bash
npm install -g AWMate-helicone-session
```
...
```json title="awmate.json"
{
  "plugin": ["AWMate-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each AWMate conversation listed as a separate session.
...
You can configure AWMate to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="awmate.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://awmate.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure AWMate to use local models through LM Studio.

```json title="awmate.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://awmate.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure AWMate to use local models through Ollama.

:::tip
Ollama can automatically configure itself for AWMate. See the [Ollama integration docs](https://docs.ollama.com/integrations/awmate) for details.
:::

```json title="awmate.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://awmate.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with AWMate:
...
3. Copy the API key for use in AWMate.
...
6. **Important**: Before using cloud models in AWMate, you must pull the model information locally:
...
AWMate Zen is a list of tested and verified models provided by the AWMate team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>AWMate Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **AWMate Zen**.

   ```txt
   /connect
   ```

3. Enter your AWMate API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your AWMate config.
...
5. You can also customize them through your AWMate config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' AWMate
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group AWMate
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with AWMate:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=AWMate&utm_medium=docs&utm_campaign=cortex-provider).

AWMate's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. AWMate captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting AWMate:
...
The model catalog is provided automatically. A minimal `awmate.json` is all that's needed:
...
You can also customize models through your AWMate config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. AWMate opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
AWMate refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when AWMate is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running AWMate — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. AWMate prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. AWMate polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with AWMate. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in awmate.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `awmate.json` file in your project directory:

   ```json title="awmate.json" ""myprovider"" {5-15}
   {
     "$schema": "https://awmate.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="awmate.json" {9,11,17-20}
{
  "$schema": "https://awmate.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow AWMate to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `awmate auth list` to see if the credentials
...
2. For custom providers, check the AWMate config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your AWMate config.
