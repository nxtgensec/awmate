...
Existem dois tipos de agentes no AWMate; agentes primários e subagentes.
...
AWMate vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
AWMate vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

AWMate vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do AWMate, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `awmate.json`:
...
- Global: `~/.config/awmate/agents/`
- Por projeto: `.awmate/agents/`

```markdown title="~/.config/awmate/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o AWMate usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do AWMate quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do AWMate usa o formato `provider/model-id`. Por exemplo, se você estiver usando [AWMate Zen](/docs/zen), você usaria `awmate/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="awmate.json" {3-6,9-12}
{
  "$schema": "https://awmate.ai/config.json",
  "tools": {
    "write": true,
...
```json title="awmate.json" {3-5,8-10}
{
  "$schema": "https://awmate.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `AWMate models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/awmate).
:::
...
```markdown title="~/.config/awmate/agents/docs-writer.md"
---
...
```markdown title="~/.config/awmate/agents/security-auditor.md"
---
