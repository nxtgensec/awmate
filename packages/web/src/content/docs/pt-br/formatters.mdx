---
title: Formatadores
description: O AWMate usa formatadores específicos de linguagem.
---

O AWMate formata automaticamente arquivos após serem escritos ou editados usando formatadores específicos de linguagem. Isso garante que o código gerado siga os estilos de código do seu projeto.

---

## Integrados

O AWMate vem com vários formatadores integrados para linguagens e frameworks populares. Abaixo está uma lista dos formatadores, extensões de arquivo suportadas e comandos ou opções de configuração necessárias.
...
Portanto, se seu projeto tiver `prettier` em seu `package.json`, o AWMate o usará automaticamente.

---

## Como funciona

Quando o AWMate escreve ou edita um arquivo, ele:
...
Você pode personalizar os formatadores através da seção `formatter` em sua configuração do AWMate.
