...
Você pode adicionar ferramentas externas ao AWMate usando o _Model Context Protocol_, ou MCP. O AWMate suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do AWMate](https://awmate.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/awmate`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O AWMate lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o AWMate irá:
...
Se o servidor requer autenticação, o AWMate solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `awmate mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o AWMate armazenará os tokens de forma segura em `~/.local/share/awmate/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no AWMate, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do AWMate como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o AWMate à sua conta do Sentry.
