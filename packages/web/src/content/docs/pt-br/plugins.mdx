---
title: Plugins
description: Escreva seus próprios plugins para estender o AWMate.
---

Plugins permitem que você estenda o AWMate conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do AWMate.
...
- `.awmate/plugins/` - Plugins em nível de projeto
- `~/.config/awmate/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/awmate/node_modules/`.
...
1. Configuração global (`~/.config/awmate/awmate.json`)
2. Configuração do projeto (`awmate.json`)
3. Diretório de plugins global (`~/.config/awmate/plugins/`)
4. Diretório de plugins do projeto (`.awmate/plugins/`)
...
O AWMate executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do AWMate para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o AWMate.
...
:::note
Se você estiver usando o aplicativo desktop AWMate, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o AWMate de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao AWMate:
...
O helper `tool` cria uma ferramenta personalizada que o AWMate pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o AWMate junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://awmate.ai/docs/sdk) para detalhes.
