---
title: Servidor
description: Interaja com o servidor AWMate via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `AWMate serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente AWMate pode usar.
...
```bash
AWMate serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `AWMate.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
AWMate serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `AWMate_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `AWMate`, ou defina `AWMate_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `AWMate serve` quanto ao `AWMate web`.

```bash
AWMate_SERVER_PASSWORD=your-password AWMate serve
```
...
Quando você executa `AWMate`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor AWMate para interagir com o AWMate programaticamente.
:::

Essa arquitetura permite que o AWMate suporte múltiplos clientes e permite que você interaja com o AWMate programaticamente.

Você pode executar `AWMate serve` para iniciar um servidor autônomo. Se você tiver o TUI do AWMate em execução, `AWMate serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do AWMate [IDE](/docs/ide).
...
O servidor AWMate expõe as seguintes APIs.
