...
As habilidades do agente permitem que o AWMate descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O AWMate pesquisa nesses locais:

- Configuração do projeto: `.awmate/skills/<name>/SKILL.md`
- Configuração global: `~/.config/awmate/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o AWMate sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.awmate/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/awmate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.awmate/skills/git-release/SKILL.md` assim:
...
O AWMate lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `awmate.json`:
...
**Para agentes embutidos** (em `awmate.json`):
