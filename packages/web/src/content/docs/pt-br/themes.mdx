...
Com o AWMate, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o AWMate usa nosso próprio tema `AWMate`.
...
O AWMate vem com vários temas embutidos.
...
- Querem que o AWMate corresponda à aparência do seu terminal
...
O AWMate suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/awmate/themes/*.json` ou `$XDG_CONFIG_HOME/awmate/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.awmate/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.awmate/themes/*.json`
