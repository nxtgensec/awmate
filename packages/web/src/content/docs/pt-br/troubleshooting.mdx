...
Para depurar problemas com o AWMate, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\AWMate\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `AWMate --log-level DEBUG`.

---

## Armazenamento

O AWMate armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/awmate/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\AWMate`
...
O AWMate Desktop executa um servidor AWMate local (o sidecar `AWMate-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `AWMate` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/awmate/awmate.jsonc` (ou `~/.config/awmate/awmate.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/awmate/awmate.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\AWMate\awmate.jsonc`
...
O AWMate também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/awmate/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\AWMate\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o AWMate possa reconstruí-lo.

1. Saia completamente do AWMate Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/awmate`
- **Linux**: exclua `~/.cache/awmate` (ou execute `rm -rf ~/.cache/awmate`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\AWMate`

3. Reinicie o AWMate Desktop.

---

### Corrigindo problemas de conexão com o servidor

O AWMate Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `awmate.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `AWMate_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `AWMate_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o AWMate Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do AWMate.

---

### Notificações não aparecendo

O AWMate Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o AWMate nas configurações do seu sistema operacional, e
...
1. Saia do AWMate Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo AWMate Desktop):

- `AWMate.settings.dat` (URL do servidor padrão do desktop)
- `AWMate.global.dat` e `AWMate.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o AWMate:
...
3. Certifique-se de que você tem a versão mais recente com `AWMate upgrade`
...
Para descobrir quais modelos você tem acesso, execute `AWMate models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\AWMate`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O AWMate instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\AWMate`

2. Reinicie o AWMate para reinstalar os pacotes de provedores mais recentes

Isso forçará o AWMate a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O AWMate detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
