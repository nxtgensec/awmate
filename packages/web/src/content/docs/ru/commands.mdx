...
Создайте `.awmate/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию AWMate или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем AWMate [config](/docs/config):
...
- Глобальный: `~/.config/awmate/commands/`
- Для каждого проекта: `.awmate/commands/`
...
AWMate включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
