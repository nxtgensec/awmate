---
title: Server
description: Interact with AWMate server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `AWMate serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an AWMate client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `AWMate.local` |
...
Set `AWMate_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `AWMate`, or set `AWMate_SERVER_USERNAME` to override it. This applies to both `AWMate serve` and `AWMate web`.
...
When you run `AWMate` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the AWMate server to interact with AWMate programmatically.
:::

This architecture lets AWMate support multiple clients and allows you to interact with AWMate programmatically.

You can run `AWMate serve` to start a standalone server. If you have the
AWMate TUI running, `AWMate serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the AWMate [IDE](/docs/ide) plugins.
...
The AWMate server exposes the following APIs.
