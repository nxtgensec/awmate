...
With AWMate you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, AWMate uses our own `AWMate` theme.
...
AWMate comes with several built-in themes.
...
- Want AWMate to match their terminal's appearance
...
AWMate supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/awmate/themes/*.json` or `$XDG_CONFIG_HOME/awmate/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.awmate/themes/*.json`
4. **Current working directory** - Defined in `./.awmate/themes/*.json`
