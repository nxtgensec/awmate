...
`.awmate/commands/test.md` oluştur:
...
AWMate işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

AWMate [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/awmate/commands/`
- Proje başına: `.awmate/commands/`
...
AWMate, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
