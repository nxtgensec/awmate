...
AWMate Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, AWMate'daki diğer sağlayıcılar gibi çalışır. AWMate Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve AWMate'u kullanmak için bunu kullanmanıza gerek yoktur.
...
AWMate Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

AWMate Go, AWMate'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>AWMate Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `AWMate Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye AWMate Go'ya abone olabilir.
:::
...
AWMate Go aşağıdaki limitleri içerir:
...
AWMate yapılandırmanızdaki [model id](/docs/config/#models) formatı `AWMate-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `AWMate-go/kimi-k2.7-code` kullanmalısınız.
...
AWMate Go'yu şu amaçlarla oluşturduk:
...
4. AWMate ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
