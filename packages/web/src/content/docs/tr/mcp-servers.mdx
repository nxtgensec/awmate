...
_Model Bağlam Protokolü_ veya MCP kullanarak AWMate'a harici araçlar ekleyebilirsiniz. AWMate hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [AWMate Config](https://awmate.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/awmate` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
AWMate, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde AWMate şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa AWMate, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `awmate mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından AWMate, belirteçleri `~/.local/share/awmate/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra AWMate'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi AWMate yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve AWMate'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
