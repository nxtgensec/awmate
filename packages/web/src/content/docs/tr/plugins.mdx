---
title: Eklentiler
description: AWMate'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek AWMate'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya AWMate'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.awmate/plugins/` - Proje düzeyinde eklentiler
- `~/.config/awmate/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/awmate/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/awmate/awmate.json`)
2. Project config (`awmate.json`)
3. Global eklenti dizini (`~/.config/awmate/plugins/`)
4. Proje eklenti dizini (`.awmate/plugins/`)
...
AWMate bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir AWMate SDK istemcisi.
...
AWMate'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
AWMate masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
AWMate'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca AWMate'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, AWMate'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra AWMate'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://awmate.ai/docs/sdk) bakın.
