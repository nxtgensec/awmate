---
title: Sağlayıcılar
description: AWMate'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

AWMate, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı AWMate yapılandırmanızda yapılandırın.
...
`~/.local/share/awmate/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları AWMate'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
AWMate Zen, AWMate ekibi tarafından sağlanan modellerin bir listesidir.
AWMate ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz AWMate Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, AWMate'u seçin ve [AWMate.ai/auth](https://awmate.ai/auth)'ye gidin.
...
AWMate'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## AWMate Go

AWMate Go, AWMate ile iyi çalıştığı test edilmiş ve doğrulanmış, AWMate ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `AWMate Go`'yu seçin ve [AWMate.ai/auth](https://awmate.ai/zen) adresine gidin.
...
Amazon Bedrock'u AWMate ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   AWMate'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `awmate.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi AWMate'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
AWMate'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   AWMate'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   AWMate yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
AWMate, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. AWMate'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`awmate.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [AWMate-gitlab-auth](https://www.npmjs.com/package/awmate-gitlab-auth) ana sayfasında.

##### Yapılandırma

`awmate.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi AWMate'la kullanmak için:
...
Google Vertex AI'yi AWMate ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları AWMate çalıştırılırken ayarlayın.
...
Helicone'un AWMate aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her AWMate görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [AWMate-helicone-session](https://github.com/H2Shami/awmate-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her AWMate görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
AWMate'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla AWMate'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla AWMate'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini AWMate için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/awmate)'a bakın.
:::
...
Ollama Cloud'u AWMate ile kullanmak için:
...
3. AWMate'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: AWMate'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
AWMate Zen, AWMate ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>AWMate Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **AWMate Zen**'i arayın.

   ```txt
   /connect
   ```

3. AWMate API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   AWMate yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca AWMate yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi AWMate ile kullanmak için:
...
Modelleri AWMate yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı AWMate ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `awmate.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları AWMate'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `awmate auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için AWMate yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, AWMate yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
