...
Ajan becerileri, AWMate'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
AWMate su konumlari tarar:

- Proje konfigurasyonu: `.awmate/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/awmate/skills/<name>/SKILL.md`
...
Proje yerel yollarinda AWMate, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.awmate/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/awmate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.awmate/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
AWMate, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`awmate.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`awmate.json` icinde):
