...
AWMate ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak AWMate kendi `AWMate` temasını kullanır.
...
AWMate birden fazla yerleşik temayla gelir.
...
- AWMate'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
AWMate, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/awmate/themes/*.json` veya `$XDG_CONFIG_HOME/awmate/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.awmate/themes/*.json`
4. **Geçerli çalışma dizini** - `./.awmate/themes/*.json`
