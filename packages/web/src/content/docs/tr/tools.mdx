...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. AWMate yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
AWMate içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `AWMate_EXPERIMENTAL_LSP_TOOL=true` (veya `AWMate_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca AWMate provider kullanılırken veya `AWMate_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

AWMate başlatırken etkinleştirmek için:
