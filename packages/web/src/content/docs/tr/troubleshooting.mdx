...
AWMate ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\AWMate\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `AWMate --log-level DEBUG`.

---

## Depolama

AWMate, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/awmate/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\AWMate` yapistirin
...
AWMate Desktop arka planda yerel bir AWMate sunucusu (`AWMate-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `AWMate` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/awmate/awmate.jsonc` (veya `~/.config/awmate/awmate.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/awmate/awmate.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\AWMate\awmate.jsonc` yapistirin
...
AWMate diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/awmate/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\AWMate\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), AWMate'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. AWMate Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/awmate`
- **Linux**: `~/.cache/awmate` dizinini silin (veya `rm -rf ~/.cache/awmate` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\AWMate` yapistirin

3. AWMate Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

AWMate Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`awmate.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `AWMate_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `AWMate_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta AWMate Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, AWMate ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

AWMate Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda AWMate icin bildirimler etkinse
...
1. AWMate Desktop'u kapatin
2. Su dosyalari bulun ve silin (AWMate Desktop uygulama veri dizininde yer alirlar):

- `AWMate.settings.dat` (desktop varsayilan sunucu URL'si)
- `AWMate.global.dat` ve `AWMate.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
AWMate ile ilgili bir sorun yasiyorsaniz:
...
3. `AWMate upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `AWMate models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\AWMate`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. AWMate, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\AWMate`

2. En guncel provider paketlerini yeniden kurmak icin AWMate'u yeniden baslatin

Bu, AWMate'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
AWMate, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
