...
const model = "awmate/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/awmate")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with awmate...`)
  ...
  try {
    await $`awmate run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  awmate failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with awmate")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with awmate...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/awmate`.",
