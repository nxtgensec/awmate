...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "awmate/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".awmate/glossary/zh-cn.md"
  if (locale === "zht") return ".awmate/glossary/zh-tw.md"
  return `.awmate/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("AWMate did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("AWMate returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("AWMate session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel AWMate runs for 'all' (default: 4)
      --model <provider/id>  AWMate model (default: awmate/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running AWMate
...
`)
  ...
  if (failed.length) console.error(`\nAWMate failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.AWMATE_DISABLE_PROJECT_CONFIG = "1"
  env.AWMATE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["awmate", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.AWMATE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["awmate", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.AWMATE_CONFIG
  delete env.AWMATE_CONFIG_DIR
  delete env.AWMATE_CONFIG_CONTENT
  delete env.AWMATE_PERMISSION
  delete env.AWMATE_AUTO_SHARE
  return env
}
