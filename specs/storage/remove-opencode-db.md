# Remove `packages/awmate/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/awmate/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the awmate legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/awmate/src/storage/db.ts` are concentrated in 22 source files:

- `packages/awmate/src/account/repo.ts`
- `packages/awmate/src/cli/cmd/db.ts`
- `packages/awmate/src/cli/cmd/import.ts`
- `packages/awmate/src/cli/cmd/stats.ts`
- `packages/awmate/src/control-plane/workspace.ts`
- `packages/awmate/src/index.ts`
- `packages/awmate/src/node.ts`
- `packages/awmate/src/permission/index.ts`
- `packages/awmate/src/project/project.ts`
- `packages/awmate/src/server/projectors.ts`
- `packages/awmate/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/awmate/src/server/shared/fence.ts`
- `packages/awmate/src/session/message-v2.ts`
- `packages/awmate/src/session/projectors.ts`
- `packages/awmate/src/session/prompt.ts`
- `packages/awmate/src/session/session.ts`
- `packages/awmate/src/session/todo.ts`
- `packages/awmate/src/share/share-next.ts`
- `packages/awmate/src/storage/db.ts`
- `packages/awmate/src/sync/index.ts`
- `packages/awmate/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy awmate database wrapper; `packages/awmate/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the awmate wrapper.
...
Status: Completed. `SyncEvent` and the awmate projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy awmate database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy awmate database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy awmate database wrapper.
...
All migration groups are complete or superseded. `packages/awmate/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No awmate data-migration group remains.

The previous awmate `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate awmate data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/awmate`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/awmate/src`
- `bun typecheck` from `packages/awmate`
- Relevant package tests from `packages/awmate`, not the repo root
