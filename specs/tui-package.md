...
Move the canonical AWMate terminal application from
`packages/awmate/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/awmate`,
`packages/cli`, or `@awmate-ai/core`.

The SDK is the TUI's AWMate boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/awmate`.
- Do not replace private `packages/awmate` imports with `@awmate-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@awmate-ai/tui`.
...
- AWMate domain data displayed by the TUI
...
```text
packages/awmate/src/cli/cmd/tui
```

Its private dependency on `packages/awmate` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/awmate/src/*`.
...
Status: Completed. The private `@awmate-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@awmate-ai/tui`.
...
- The package has no dependency on `awmate`, `@awmate-ai/cli`, or
  `@awmate-ai/core`.
...
primitives now live in `@awmate-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@awmate-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads AWMate globals or process state;
...
Replace process-global AWMate state with resolved TUI inputs.
...
`@awmate-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/awmate` initially.
...
canonical in `@awmate-ai/tui`. Configured references resolve through the new
...
adapters rather than AWMate domain implementations.

Make the SDK the only AWMate domain boundary used by the TUI.
...
- Domain-facing TUI code imports AWMate data and operations only from
  `@awmate-ai/sdk`.
...
`@awmate-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/awmate`.
...
- The package has no imports from `packages/awmate`, `packages/cli`, or
  `@awmate-ai/core`.
...
public `@awmate-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/awmate`.
- Keep the legacy embedded worker and server startup in `packages/awmate`.
- Change those adapters to load config, create transport inputs, and call the
  public `@awmate-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/awmate`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@awmate-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@awmate-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for AWMate domain state.
...
cd packages/awmate && bun typecheck
...
rg '@awmate-ai/core|packages/awmate|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/awmate/src packages/awmate/test
```
...
- Build the current-platform `packages/awmate` binary.
