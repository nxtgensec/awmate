...
Both `action` and `resource` use awmate's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.awmate` files is intentionally deferred until `.awmate` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether awmate permits an operation through its managed execution paths.
