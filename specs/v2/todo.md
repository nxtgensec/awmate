...
The awmate server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@awmate-ai/llm` tool loop after replacing its
...
- plugins get global "awmate" instance like in that post i showed
- awmate instance has stuff like `awmate.session.prompt()` or
  `awmate.tool.register({...})`
...
- decide whether to preserve deprecated `@awmate-ai/llm` orchestration exports
